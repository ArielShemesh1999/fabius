...
**n8n-mcp** (`czlonkowski/n8n-mcp`, MIT) is the headline: an MCP server giving the agent structured access to 2,100+ n8n nodes + schemas, so it **looks up the real node config and validates before emitting JSON** instead of hallucinating params — the skill's *prove-the-wiring-before-live* gate. This beats the negligible-adoption n8n-workflow-generator fine-tunes on HF; don't reach for a model where a schema will do.
