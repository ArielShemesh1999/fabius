...
`fabius-archivum` (the knowledge engine, LLM-wiki, RAG pipeline in `references/knowledge/`), `fabius-parcus` (*when-to-add-vector* — don't stand up a store a small vault doesn't need), **Fabius Bidi** / **Fabius Yisrael** (why the default embedder must handle Hebrew), and `fabius-cohors` (a retrieval tool the agent calls).
