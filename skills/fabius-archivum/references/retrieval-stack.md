...
- **PyLate** (MIT) — train/index/retrieve with **ColBERT late-interaction** (token-level), which generalizes better to niche domains and **low-resource languages like Hebrew** than single-vector recall. It is the **maintained** library on this line — **v1.6.0, published 2026-06-11** — so it is the one to build on. *(RAGatouille was the friendlier wrapper and is still where most tutorials point, but it has shipped nothing since **0.0.9.post2 · 2025-05-19** (last commit 2025-05-17, ~98 open issues). **The trap: the repo is not archived and carries no deprecation notice**, so the usual "is this dead?" glance comes back green — read the **last release date**, never the badge. Downstream already moved: the `llama-index-packs-ragatouille-retriever` pack is marked **DEPRECATED — no longer maintained** on PyPI. Treat RAGatouille as read-only history, not a dependency.)*
- **Contextualize at write time, not just rerank at read time.** A chunk cut out of its document loses the referents that made it findable — *"revenue grew 3%"* answers to nobody. Before embedding **and** before building the BM25 index, prepend **50–100 tokens** situating the chunk in its source (which document, which decision, which period, what preceded it). This is the largest single cut in top-20 retrieval misses available on this page — ~**35%** from contextual embeddings, ~**49%** once the lexical leg is contextualized too, ~**67%** with the rerank stage stacked on top — so it *multiplies* with the rerank below rather than replacing it. Generated against the whole document with prompt caching it is a one-off cost of roughly **$1 per million document tokens**. A `file > heading` breadcrumb is the free approximation and it stops paying the moment one heading (`Decisions`, `Gotchas`) repeats across pages — which in a vault is immediately. The bundled pipeline in [`knowledge/rag/`](knowledge/rag/) prepends that breadcrumb and nothing more: treat it as the floor, not the ceiling.
...
| **Qwen/Qwen3-VL-Embedding** (2B/8B) + **Qwen3-VL-Reranker-8B** | Apache-2.0 | The commercially-clean door to *visual* retrieval: text, images, document pages and video in **one** space at 32k context, with a matched cross-encoder so both stages align. Reach for it wherever ColPali's backbone terms are the blocker — but it inherits ~**30** languages, not bge-m3's 100+, so **prove Hebrew on your own pages before committing**. |
| **vidore/colpali-v1.3** | ⚠️ **Gemma license** (backbone) | *Visual* retrieval — embeds PDF/scanned **page images** directly (skips OCR); great for scanned Hebrew documents. The license, stated exactly: the **adapter weights are MIT**, the **PaliGemma backbone they run on is Gemma-licensed** — you cannot use one without the other, so Gemma terms govern the deployment. Needs a GPU. `vidore/colqwen2-v1.0` is the smaller, older Apache-2.0 escape hatch. |
...
- **Docsify** (`docsifyjs/docsify`) · **MIT** · ~31k★ · **v5.0.0 stable** (published 2026-07-23; the npm `latest` tag) — publish the markdown vault as a **browsable, searchable site with zero build**: one `index.html` (CDN script) over a folder of `.md`, client-side full-text search, themes, straight onto GitHub Pages. The lean KB viewer that travels *beside* the vault — no SSG build pipeline to stand up (`fabius-parcus`: don't add a build a folder-of-markdown doesn't need). **Take v5 on anything new**, and mind the two v4→v5 edges: the theme path moved to `docsify@5/dist/themes/core.min.css` (v4 served themes from `lib/`), and `latest` now resolves to the 5 line — so **pin the major** in the CDN URL or an unpinned `@latest` crosses a major version on its own. The v4 line has had no release since **v4.13.1 (2023-06-24)**: keep it only for a site already pinned to it, never for a new vault. **Caveats unchanged:** it renders **client-side**, so there's no static HTML artifact → weaker SEO / AI-crawler visibility than a pre-rendered SSG unless you add prerender (`fabius-mercatus`); and it renders arbitrary markdown/HTML in the browser → **an XSS surface for untrusted content** (sanitize; `fabius-praesidium`).
