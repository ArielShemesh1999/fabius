...
from the-vector-engine.llama_index import TurboQuantVectorStore
...
`query_embedding` is **required**. the-vector-engine doesn't embed query text itself; the calling component (retriever / query engine) is responsible for that.
...
- **MMR is not supported.** Max-marginal-relevance retrieval requires the full-precision embedding of each candidate to compute pairwise diversity; the-vector-engine discards full-precision vectors after quantization.
