...
`the-vector-engine.agno.TurboQuantVectorDb` is an [Agno](https://github.com/agno-agi/agno) `VectorDb` backed by an `IdMapIndex`. It implements the same public surface as `agno.vectordb.lancedb.LanceDb` (the closest in-tree single-machine backend) so this can be swapped in wherever LanceDb is used.

## Install

```bash
pip install the-vector-engine[agno]
```
...
from the-vector-engine.agno import TurboQuantVectorDb

vector_db = TurboQuantVectorDb(embedder=OpenAIEmbedder())

knowledge = Knowledge(vector_db=vector_db)
knowledge.load_text("The vector engine compresses vectors to 4 bits per dimension.")

agent = Agent(knowledge=knowledge)
agent.print_response("What does the-vector-engine do?")
```
...
| `search_type` | Only `SearchType.vector` is supported. Constructing with `keyword` or `hybrid` raises `ValueError` (keyword/hybrid would require an external BM25/lexical index that the-vector-engine doesn't ship). |
| `distance` | Only `Distance.cosine` is supported. the-vector-engine stores unit-normalized vectors and the kernel's raw score is cosine similarity directly. |
...
- **Cosine distance only.** `Distance.cosine` is the only supported metric. the-vector-engine stores unit-normalized vectors; other distances would require non-trivial scoring changes.
