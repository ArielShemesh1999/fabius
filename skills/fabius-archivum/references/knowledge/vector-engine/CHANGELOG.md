...
All notable changes to the-vector-engine are recorded here. The format is based on
[Keep a Changelog](https://keepachangelog.com/en/1.1.0/) and the project follows
[Semantic Versioning](https://semver.org/spec/v2.0.0.html).

The Rust crate (`the-vector-engine` on crates.io) and the Python distribution
(`the-vector-engine` on PyPI) version independently. Each release section below
is split by surface — a single feature can affect both, and its bullet
appears under each surface it touches.

## [Unreleased]

## the-vector-engine 0.8.0 (Python package) + the-vector-engine 0.9.0 (Rust crate) — 2026-06-10
...
### the-vector-engine — Rust crate (current: 0.8.1 → next: 0.9.0)
...
### the-vector-engine — Python package (current: 0.7.1 → next: 0.8.0)
...
## the-vector-engine 0.7.1 (Python package) + the-vector-engine 0.8.1 (Rust crate) — 2026-06-09
...
### the-vector-engine — Rust crate (current: 0.8.0 → next: 0.8.1)
...
### the-vector-engine — Python package (current: 0.7.0 → next: 0.7.1)
...
- **Plain `cargo build` of the extension now links on macOS.** Building
  `the-vector-engine-python` from source failed with "symbol(s) not found for
...
## the-vector-engine 0.7.0 (Python package) + the-vector-engine 0.8.0 (Rust crate) — 2026-05-30
...
### the-vector-engine — Rust crate (current: 0.7.0 → next: 0.8.0)
...
### the-vector-engine — Python package (current: 0.6.0 → next: 0.7.0)
...
- **`the-vector-engine.langchain.TurboQuantVectorStore`**: `similarity_search`,
...
- **`the-vector-engine.haystack.TurboQuantDocumentStore`**: `Document.blob` and
...
- **`the-vector-engine.llama_index.TurboQuantVectorStore`**: full `BaseNode`
  ...
  - `add()` rejects intra-batch duplicate `node_id`s with a clear
    ...
    first node's vector).
- **`the-vector-engine.agno.TurboQuantVectorDb`**: `embedder` is now threaded
...
## the-vector-engine 0.6.0 (Python package) + the-vector-engine 0.7.0 (Rust crate) — 2026-05-27

### the-vector-engine — Rust crate (current: 0.6.0 → next: 0.7.0)
...
### the-vector-engine — Python package (current: 0.5.3 → next: 0.6.0)
...
## the-vector-engine 0.5.3 (Python package) + the-vector-engine 0.6.0 (Rust crate) — 2026-05-25

### the-vector-engine — Rust crate (current: 0.5.0 → next: 0.6.0)
...
- **BREAKING:** `TurboQuantIndex::new`, `TurboQuantIndex::new_lazy`,
  ...
  input. The new `the-vector-engine::ConstructError` enum covers `bit_width`
...
### the-vector-engine — Python package (current: 0.5.2 → next: 0.5.3)

#### Fixed

- **Linux wheels now actually import.** Every Linux wheel since
  Linux build support was added had a missing `DT_NEEDED` entry for
  `libopenblas`, so `import the-vector-engine` failed at the dynamic linker
  ...
  deps) into `the-vector-engine.libs/`. Linux wheel size grows from ~1.8 MB
...
## the-vector-engine 0.5.2 (Python package) + the-vector-engine 0.5.0 (Rust crate) — 2026-05-21

### the-vector-engine — Rust crate (current: 0.4.1 → next: 0.5.0)

#### Changed

- **BREAKING:** `TurboQuantIndex::add_2d`, `IdMapIndex::add_with_ids_2d`,
  and `IdMapIndex::add_with_ids` now return `Result<(), AddError>`
  instead of panicking on invalid input. The new `the-vector-engine::AddError`
...
### the-vector-engine — Python package (current: 0.5.1 → next: 0.5.2)
...
## the-vector-engine 0.5.1 (Python package) + the-vector-engine 0.4.1 (Rust crate) — 2026-05-18

### the-vector-engine — Rust crate (current: 0.4.0 → next: 0.4.1)
...
- **`the-vector-engine::search::BLOCKS_SKIPPED_BY_MASK`** — atomic counter
...
### the-vector-engine — Python package (current: 0.5.0 → next: 0.5.1)
...
> Existing `.tv` and `.tvim` files written by the-vector-engine ≤ 0.4.3 cannot
...
```python
import numpy as np
from the-vector-engine import TurboQuantIndex
...
```
this .tv file was written by the-vector-engine ≤ 0.4.3 (format version 1).
It is incompatible with the-vector-engine 0.4.4+ because the per-vector scalar's
meaning changed. Rebuild this index from the source vectors using
the-vector-engine 0.4.4 or later.
```

### the-vector-engine — Rust crate (current: 0.3.0 → next: 0.4.0)
...
### the-vector-engine — Python package (current: 0.4.3 → next: 0.5.0)
...
- **On-disk format version bumped to 2** for both `.tv` and `.tvim`.
  ...
  bumped from 1 to 2.
- **Loading a the-vector-engine ≤ 0.4.3 index raises with a clear error.**
...
- **`the-vector-engine.haystack.TurboQuantDocumentStore` clamps cosine scores
...
## the-vector-engine 0.4.3 (Python package) — 2026-05-18

### the-vector-engine — Python package (current: 0.4.2 → next: 0.4.3)
...
## the-vector-engine 0.4.2 (Python package) — 2026-05-17

### the-vector-engine — Python package (current: 0.4.1 → next: 0.4.2)
...
- **`numpy` is now a declared runtime dependency.** The Python package
  ...
  This broke `pip install the-vector-engine[agno]` in clean environments because
...
## the-vector-engine 0.4.1 (Python package) — 2026-05-17

### the-vector-engine — Python package (current: 0.4.0 → next: 0.4.1)

#### Added

- **Agno integration** (`the-vector-engine.agno`). New `TurboQuantVectorDb` class
  ...
  - Constructor restricts `search_type=vector` and `distance=cosine`
    — the-vector-engine doesn't ship a BM25/lexical index and stores
  ...
  - Install: `pip install the-vector-engine[agno]`.

## the-vector-engine 0.3.0 (Rust crate) — 2026-05-17

## the-vector-engine 0.3.0 (Rust crate) — 2026-05-17

### the-vector-engine — Rust crate (current: 0.2.0 → next: 0.3.0)
...
## the-vector-engine 0.4.0 (Python package) — 2026-05-17

### the-vector-engine — Python package (current: 0.3.0 → next: 0.4.0)
...
- **Haystack integration** (`the-vector-engine.haystack`):
  ...
  - Constructor accepts `embedding_similarity_function`
    (`"cosine"` default, since the-vector-engine stores unit-normalized
...
- **LangChain integration** (`the-vector-engine.langchain`):
  ...
  - `max_marginal_relevance_search` / `_by_vector` /
    ...
    candidate vectors that the-vector-engine discards after encoding.
  - Persistence methods renamed: `save_local → dump`, `load_local →
    load`, matching the reference.

- **LlamaIndex integration** (`the-vector-engine.llama_index`):
  `TurboQuantVectorStore` is now a structural drop-in for
