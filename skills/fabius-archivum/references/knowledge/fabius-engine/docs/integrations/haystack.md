...
from the-vector-engine.haystack import TurboQuantDocumentStore
...
- **Embeddings are not retained.** `embedding_retrieval(..., return_embedding=True)` is accepted for signature compatibility but `Document.embedding` is always `None` on retrieved docs — the-vector-engine discards the full-precision vector after quantization.
