...
Thank you for helping keep the-vector-engine and its users safe. Security reports are
...
the-vector-engine is pre-1.0 and ships frequently. Security fixes target the **latest
released version** of each surface:

- the `the-vector-engine` crate on [crates.io](https://crates.io/crates/the-vector-engine), and
- the `the-vector-engine` distribution on [PyPI](https://pypi.org/project/the-vector-engine/).
...
- behavior on unsupported targets (the-vector-engine requires a 64-bit platform and
