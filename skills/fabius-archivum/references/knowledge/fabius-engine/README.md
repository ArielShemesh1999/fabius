<p align="center">
  <img src="docs/header.png" alt="the-vector-engine — Google's TurboQuant for vector search" width="100%">
</p>
...
**A 10 million document corpus takes 31 GB of RAM as float32. the-vector-engine fits it in 4 GB - and searches it faster than FAISS.**

the-vector-engine is a Rust vector index with Python bindings, built on Google Research's [**TurboQuant**](https://arxiv.org/abs/2504.19874) algorithm — a data-oblivious quantizer with near-optimal distortion and no separate training phase.
...
```python
import numpy as np
from the-vector-engine import IdMapIndex
...
- [LangChain](docs/integrations/langchain.md) — `pip install the-vector-engine[langchain]` · replaces `langchain_core.vectorstores.InMemoryVectorStore`
- [LlamaIndex](docs/integrations/llama_index.md) — `pip install the-vector-engine[llama-index]` · replaces `llama_index.core.vector_stores.SimpleVectorStore`
- [Haystack](docs/integrations/haystack.md) — `pip install the-vector-engine[haystack]` · replaces `haystack.document_stores.in_memory.InMemoryDocumentStore`
- [Agno](docs/integrations/agno.md) — `pip install the-vector-engine[agno]` · replaces `agno.vectordb.lancedb.LanceDb`

## Rust

```bash
cargo add the-vector-engine
```

```rust
use the-vector-engine::TurboQuantIndex;
...
```rust
use the-vector-engine::IdMapIndex;
...
- [FAISS Fast accumulation of PQ and AQ codes](https://github.com/facebookresearch/faiss/wiki/Fast-accumulation-of-PQ-and-AQ-codes-(FastScan)) -- the-vector-engine's x86 SIMD kernel adapts FastScan's pack layout, nibble-LUT scoring, and u16 accumulator strategy
