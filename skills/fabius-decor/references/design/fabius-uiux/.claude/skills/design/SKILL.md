...
After generation, **ALWAYS** ask user about HTML preview via `AskUserQuestion`. If yes, invoke `/fabius-uiux` for gallery.
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses `fabius-uiux`, `brand`, `design-system`, `chrome-devtools` skills.
...
4. **Design** — `/ckm:brand` → `/ckm:design-system` → randomly invoke `/ck:fabius-uiux` OR `/ck:frontend-design`; HTML per idea × size
...
**Related Skills:** frontend-design, fabius-uiux, ai-multimodal, chrome-devtools
