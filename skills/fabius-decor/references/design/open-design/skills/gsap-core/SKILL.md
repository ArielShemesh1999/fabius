---
name: fabius-decor-gsap-core-gsap-core
description: GSAP core API — gsap.to(), from(), fromTo(), easing, stagger, defaults, gsap.matchMedia() for responsive and prefers-reduced-motion animation in any JS framework or vanilla JS.
triggers:
  - "gsap"
  - "gsap core"
  - "web animation"
...
**Context:** GSAP powers Webflow Interactions. Code generated by Webflow's interaction system is GSAP-based; when users ask about Webflow animations or interactions not behaving as expected, GSAP docs and patterns (e.g. tweens, ScrollTrigger) are relevant for debugging or customizing.
