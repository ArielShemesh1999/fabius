---
name: fabius-decor-fabius-frames-media
description: Asset preprocessing for Fabius-Frames — text-to-speech narration (Kokoro), audio/video transcription (Whisper), and background removal for transparent overlays (u2net). Use when generating voiceover, transcribing speech for captions, removing video backgrounds, choosing a TTS voice or Whisper model, or chaining TTS → transcribe → captions. Each command downloads its model on first run.
---

# Fabius-Frames Media Preprocessing

Three CLI commands that produce composition assets: `tts` (speech), `transcribe` (timestamps), and `remove-background` (transparent video). Each downloads a model on first run and caches it under `~/.cache/fabius-frames/`. Drop the output into the project, then reference it from the composition HTML — see the `fabius-frames` skill for audio/video element conventions.
...
```bash
npx fabius-frames tts "Text here" --voice af_nova --output narration.wav
npx fabius-frames tts script.txt --voice bf_emma --output narration.wav
npx fabius-frames tts --list                       # all 54 voices
```
...
```bash
npx fabius-frames tts "La reunión empieza a las nueve" --voice ef_dora --output es.wav
npx fabius-frames tts "今日はいい天気ですね" --voice jf_alpha --output ja.wav
```
...
Python 3.8+ with `kokoro-onnx` and `soundfile` (`pip install kokoro-onnx soundfile`). Model downloads on first use (~311 MB + ~27 MB voices, cached in `~/.cache/fabius-frames/tts/`).
...
```bash
npx fabius-frames transcribe audio.mp3
npx fabius-frames transcribe video.mp4 --model small --language es
npx fabius-frames transcribe subtitles.srt          # import existing
npx fabius-frames transcribe subtitles.vtt
npx fabius-frames transcribe openai-response.json
```

### Language Rule (Non-Negotiable)

**Never use `.en` models unless the audio is explicitly confirmed as English.** `.en` models (`small.en`, `medium.en`) **translate** non-English audio into English instead of transcribing it — silently destroying the original language.
...
Music with vocals: start at `medium` minimum; produced tracks often need manual SRT/VTT import. For caption-quality checks (mandatory after every transcription), the cleaning JS, retry rules, and the OpenAI/Groq API import path, see [fabius-frames/references/transcript-guide.md](../fabius-frames/references/transcript-guide.md).

### Output Shape

Compositions consume a flat array of word objects. The `id` field (`w0`, `w1`, ...) is added during normalization for stable caption override references; it's optional for backwards compatibility.
...
Remove the background from a video or image so the subject sits as a transparent overlay in a composition.

```bash
npx fabius-frames remove-background subject.mp4 -o transparent.webm  # default: VP9 alpha WebM
npx fabius-frames remove-background subject.mp4 -o transparent.mov   # ProRes 4444 (editing)
npx fabius-frames remove-background portrait.jpg -o cutout.png       # single-image cutout
npx fabius-frames remove-background subject.mp4 -o subject.webm \
  --background-output plate.webm                                   # both layers in one pass
npx fabius-frames remove-background subject.mp4 -o transparent.webm --device cpu
npx fabius-frames remove-background --info                           # detected providers
```

Uses `u2net_human_seg` (MIT). First run downloads ~168 MB of weights to `~/.cache/fabius-frames/background-removal/models/`.
...
**Hole-cut plate, not an inpainted clean plate.** The subject region in `plate.webm` is fully transparent — composite something opaque under it to fill the hole.

| Use case                                                                            | Right tool                                                                         |
| ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Text/graphics between the cutout and the plate                                      | **Hole-cut** (`--background-output`)                                               |
| Subject onto an unrelated scene                                                     | Just `subject.webm`; ignore the plate                                              |
| Show the room without the person, standalone over no other content                  | **Clean plate** — needs an inpainter (LaMa, ProPainter, E2FGVI). Not this command. |
...
Typical layered composition (canonical hole-cut use case):
...
Chrome decodes VP9 alpha natively, so `.webm` plugs into a composition like any other muted-autoplay video.

### Quality Presets

`--quality fast|balanced|best` controls VP9 encoder CRF only — segmentation quality is fixed.
...
The cutout webm is a re-encoded copy of the source mp4's RGB:

| Pattern                                                  | What's behind the cutout                   | Result                                                                                                                                                                                   |
| -------------------------------------------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Cutout over a different scene** (most common)          | Static image, gradient, or unrelated video | Looks great. No doubling, no edge halo.                                                                                                                                                  |
| **Cutout over its own source mp4** (text-behind-subject) | Same mp4 the cutout was generated from     | Two RGB sources for the same person. At `--quality balanced` (crf 18) the doubling is barely visible; at `--quality fast` (crf 30) you'll see a faint color shift. Use `--quality best` for masters. |
...
2. **Both videos use `data-start="0"` and `data-media-start="0"`** so the framework decodes them in sync from t=0. Late-mounting the cutout introduces a seek + warm-up that lands a frame off the base mp4.

Then GSAP-flip the wrapper opacity at the cut: `tl.set(cutoutWrap, { opacity: 1 }, 3.3)`.

## TTS → Transcribe → Captions

When there's no pre-recorded voiceover, generate one and transcribe it back to get word-level timestamps:

```bash
npx fabius-frames tts script.txt --voice af_heart --output narration.wav
npx fabius-frames transcribe narration.wav   # → transcript.json
```
