...
Use `window.__fabius-frames.fitTextFontSize()`:

```js
var result = window.__fabius-frames.fitTextFontSize(group.text.toUpperCase(), {
  fontFamily: "Outfit",
  fontWeight: 900,
  maxWidth: 1600,
});
...
Before building caption styles from scratch, check the registry — 15 ready-to-use caption components cover the most common styles. Install with `npx fabius-frames add <name>` and use as a sub-composition via `data-composition-src`.

```bash
npx fabius-frames catalog --tag caption-style   # list all caption components
npx fabius-frames add caption-highlight         # install a specific one
```
...
Browse all with previews: [fabius-frames.heygen.com/catalog](https://fabius-frames.heygen.com/catalog)
