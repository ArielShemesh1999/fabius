#!/usr/bin/env node
// contrast-report.mjs — Fabius-Frames contrast audit
//
...
//
// Usage:
//   node skills/fabius-frames/scripts/contrast-report.mjs <composition-dir> \
...
import { fabius-framesPackageSpec, importPackagesOrBootstrap } from "./package-loader.mjs";

// Use the producer's file server — it auto-injects the Fabius-Frames runtime
// and render-seek bridge, so raw authoring HTML works without a build step.
const packages = await importPackagesOrBootstrap(["@fabius-frames/producer", "sharp"], {
  npmPackages: [fabius-framesPackageSpec("@fabius-frames/producer"), "sharp@0.34.5"],
});
...
} = packages["@fabius-frames/producer"];
...
const OUT_DIR = resolve(args.out ?? ".fabius-frames/contrast");
