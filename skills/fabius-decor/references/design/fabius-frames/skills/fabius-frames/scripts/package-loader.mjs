...
export function fabius-framesPackageSpec(packageName) {
  const version = readBundledFabius-FramesVersion();
  if (!version) {
    throw new Error(
      [
        `Could not determine the bundled Fabius-Frames version for ${packageName}.`,
...
function resolvePackageEntry(packageName) {
  ...
  for (const base of bases) {
    ...
    try {
      return createRequire(join(normalized, "__fabius-frames_skill_loader__.cjs")).resolve(
        packageName,
      );
    } catch {
...
function readBundledFabius-FramesVersion() {
...
function readPackageVersion(packageJsonPath) {
  try {
    ...
    if (manifest.name === "fabius-frames" || manifest.name === "@fabius-frames/cli") {
...
async function confirmBootstrap(packageSpecs) {
  ...
  try {
    const answer = await rl.question(
      [
        "Fabius-Frames helper package(s) are missing.",
...
function bootstrapWithNpmInstall(packageNames) {
  const installRoot = mkdtempSync(join(tmpdir(), "fabius-frames-skill-deps-"));
