---
name: fabius-decor-lottie
description: Lottie and dotLottie adapter patterns for Fabius-Frames. Use when embedding lottie-web JSON animations, .lottie files, @lottiefiles/dotlottie-web players, registering instances on window.__hfLottie, or making After Effects exports deterministic in Fabius-Frames.
---

# Lottie for Fabius-Frames

Fabius-Frames can seek both `lottie-web` and dotLottie players through its `lottie` runtime adapter. Lottie is a strong fit because the animation timeline is already encoded in the asset; Fabius-Frames only needs a player object it can seek.
...
Fabius-Frames seeks them all to the same composition time.
...
- Translating Remotion Lottie usage into plain Fabius-Frames HTML.
...
- Loading a player asynchronously and registering it after Fabius-Frames validation has already inspected the page.
...
- Fabius-Frames adapter source: `packages/core/src/runtime/adapters/lottie.ts`
- lottie-web `loadAnimation` options: https://github.com/airbnb/lottie-web/wiki/loadAnimation-options
- dotLottie web player methods: https://developers.lottiefiles.com/docs/dotlottie-player/dotlottie-web/methods
