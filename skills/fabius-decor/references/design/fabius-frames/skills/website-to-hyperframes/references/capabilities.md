# Fabius-Frames — Complete Capabilities Inventory

Everything possible in Fabius-Frames as of today's workspace, synthesized from direct source reads of all 7 packages, 16 skills, and the full registry.
...
- **Linter:** 60+ rules. Run `npx fabius-frames lint` before render. Catches missing timelines, overlapping clips, broken paths, GSAP errors.
...
| 15  | **Player web component**                             | `<fabius-frames-player>` with seek/play/pause API, 11 events, media mirror, runtime auto-inject                                                                                                                                   |
...
| 22  | **Skills (16)**                                      | fabius-frames, cli, media, registry, contrast, animation-map, website-to-fabius-frames, remotion, gsap, animejs, css-animations, waapi, lottie, three, tailwind, contribute-catalog                                                 |
...
- Looping handled by `<fabius-frames-player>`, not GSAP `repeat: -1`
...
| ---------------------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| GSAP (createGsapAdapter)           | The primary timeline + all tweens registered on `window.__timelines[<id>]`    | CDN `https://cdn.jsdelivr.net/npm/gsap@3.14.2/dist/gsap.min.js` | Plugins via standard GSAP register; Fabius-Frames does NOT patch THREE.Clock (uses `__hfThreeTime` instead) |
...
- **MotionPathPlugin** — curve-constrained tweens (skills/fabius-frames/references/techniques.md)
- **CustomEase** — bezier eases imported from Remotion-style timing
- **ScrollTrigger / Flip / SplitText / Draggable / Inertia / Observer / ScrambleText / CustomWiggle / CustomBounce / ScrollSmoother / GSDevTools** — work natively if loaded and tweens are on the registered paused timeline, but no special Fabius-Frames adapter
- Producer injects ScrollTrigger CDN automatically when needed (packages/producer/src/services/htmlCompiler.ts)

---

## 3. Shader transitions — @fabius-frames/shader-transitions
...
Documented in skills/fabius-frames/references/transitions/ across 14 category files. All GSAP-driven, none mixable with shader transitions in same composition.
...
| ------------------------------------------ | -------- | ----------------- |
| fabius-frames transcribe (local whisper.cpp) | JSON     | Word-level        |
...
| fabius-frames tts → transcribe chain         | wav→json | Word-level        |

### Positioning helpers

- Landscape: bottom 80–120px centered
- Portrait: ~600–700px from bottom
- `window.__fabius-frames.fitTextFontSize(text, {maxWidth, fontFamily, fontWeight})` for dynamic sizing
...
Documented in skills/fabius-frames/references/html-in-canvas-patterns.md (504 lines).
...
(From skills/fabius-frames/references/techniques.md)
...
- Fabius-Frames extracts frames at render via videoFrameInjector (avoids unreliable headless `<video>` playback)
...
Install: `npx fabius-frames add <name>` for blocks/components, `fabius-frames init <dir> --example <name>` for examples.
...
| publish           | Zip + upload + return fabius-frames.dev URL                                                                                                                                                                                                                                                                                |
...
| skills            | Run `npx skills add heygen-com/fabius-frames --all`                                                                                                                                                                                                                                                                        |
...
### Website capture (`fabius-frames capture <url>`)

Detects these libraries on captured sites (for context labeling): GSAP / ScrollTrigger, Three.js, Lottie, Anime.js, PixiJS, Babylon.js, Rive, Matter.js, Lenis, Framer Motion, Tailwind CSS, WebGL (shader fingerprinting). Captured outputs feed the website-to-fabius-frames skill workflow.
...
## 15. Player — `<fabius-frames-player>` web component
...
Loads `RUNTIME_CDN_URL` (`@fabius-frames/core/dist/hyperframe.runtime.iife.js`) if missing `__hf`/`__player` but timelines exist.
...
Access via `window.__fabius-frames.getVariables()`. Override at render time:

```bash
npx fabius-frames render --variables '{"brand":"Linear","primary":"#5E6AD2"}'
npx fabius-frames render --variables-file vars.json
npx fabius-frames render --strict-variables  # error if unused / mismatched
```
...
| Global                                | Purpose                                                                 |
| ------------------------------------- | ----------------------------------------------------------------------- |
| `__fabius-frames`                       | `{ fitTextFontSize, getVariables }`                                     |
...
| Skill                     | Purpose                                                                 |
| ------------------------- | ----------------------------------------------------------------------- |
| fabius-frames               | Core framework skill (composition authoring)                            |
| fabius-frames-cli           | All CLI commands as an agent skill                                      |
| fabius-frames-media         | Media workflows (TTS, transcribe, captions integration)                 |
| fabius-frames-registry      | Installing blocks/components                                            |
| fabius-frames-contrast      | WCAG audit (scripts/contrast-report.mjs)                                |
| fabius-frames-animation-map | Per-tween bbox + flags report                                           |
| website-to-fabius-frames    | Capture → DESIGN.md → brief → storyboard+script → VO → build → validate |
| remotion-to-fabius-frames   | Migration patterns + API map + CustomEase                               |
...
## 23. References inventory (skills/fabius-frames/references/)
...
**Guides:** 4k-rendering, claude-design, common-mistakes, deploy, gsap-animation, hdr, html-in-canvas, fabius-frames-vs-remotion, fabius-design, performance, prompting, remove-background, rendering, timeline-editing, troubleshooting, video-editor-cheatsheet, website-to-video, etc.
