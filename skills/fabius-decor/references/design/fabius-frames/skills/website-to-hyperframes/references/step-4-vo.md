...
npx fabius-frames tts "First sentence. Second sentence." --voice af_nova --output /tmp/test-tts.wav
...
```bash
npx fabius-frames tts SCRIPT.md --voice af_nova --output narration.wav
```
...
3. **Try a different tool for the same task** — if `fabius-frames transcribe` hangs, try `whisper-cli` directly, or Groq API (`npx fabius-frames transcribe --provider groq`), or OpenAI API
...
```bash
npx fabius-frames transcribe narration.wav
```
