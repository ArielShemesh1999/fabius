...
Load the `fabius-frames` skill — it has the rules for data attributes, timeline contracts, deterministic rendering, and layout. Read it now if you haven't already this session.
...
```bash
cp -r skills/website-to-fabius-frames/assets/sfx/ <project-dir>/sfx/
# If skill is installed elsewhere:
find . -path "*/website-to-fabius-frames/assets/sfx" -exec cp -r {} <project-dir>/sfx/ \;
```
...
the skill lives outside it — run this to find it:
  ...
  find "$HOME" -path '*/website-to-fabius-frames/references/beat-builder-guide.md' -maxdepth 10 2>/dev/null | head -1
...
and file references. Follow its workflow exactly:
  build → lint (`npx fabius-frames lint .`)
        → snapshot (`npx fabius-frames snapshot . --frames 3`)
