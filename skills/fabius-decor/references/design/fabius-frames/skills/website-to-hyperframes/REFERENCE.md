---
name: fabius-decor-website-to-fabius-frames
description: Turn any website URL into a professional Fabius-Frames video — social ads, product tours, brand reels, and launch teasers. Trigger when a user shares a URL and wants any kind of video output.
---

# Website to Fabius-Frames
...
**Autonomous mode is NOT "skip all gates."** Auto mode covers user-preference questions (TTS provider, voice, color emphasis, beat count, music yes/no, captions yes/no). It does NOT cover quality-verification gates. The following remain non-skippable in auto mode:

- Asset Audit (Step 3) — view contact sheets, justify USE/SKIP for each asset
- Per-beat HTML read (Step 5) — structured evidence block per beat
- DoD checklist (Step 6) — animation-map, per-warning WCAG verification, audio/motion playback
- Honest disclosure section (Step 6) — "What I did NOT verify" must appear in the final summary
...
Write DESIGN.md — a brand cheat sheet covering visual identity: colors, typography, component styles, layout principles. Use `design-styles.json` for exact computed values.

**Speed option:** For fast-pacing videos (billboard-per-beat), DESIGN.md can be a 50-line summary of colors + fonts + do's/don'ts. The sub-agent prompt in Step 5 pastes brand values directly, so DESIGN.md depth only matters for complex compositions.

**Gate:** `DESIGN.md` exists with at minimum: color palette, font choices, and do's/don'ts.
...
Write the storyboard concept-first: message → narrative arc → beats that serve the arc → techniques per beat → brand accents pass at the end. Then write the narration script to match. Present both to the user with a beat-by-beat summary. Iterate until approved.
...
**Read:** The `fabius-frames` skill (load it — every rule matters)
**Read:** [references/step-5-build.md](references/step-5-build.md)

Build index.html and compositions following the architecture and pacing chosen in the storyboard (Step 3). Sub-agents run `fabius-frames lint` and `fabius-frames snapshot` on each beat before reporting back.
...
**Deliver something you're proud of.** Before handing off, ask: would I post this on social media with my name on it? If not, fix what's wrong.

**Gate:** `npx fabius-frames lint` and `npx fabius-frames validate` pass with zero errors, and the final response includes the active Studio project URL.
...
Typical constraints by video type — a starting point, not a formula. Beat count follows from content and narration, not a target range.
...
Beat count is intentionally absent from this table — it comes from the storyboard. A social ad for a complex product might need 5 well-timed beats; a brand reel with one strong visual thesis might need 3.
...
| [capabilities.md](references/capabilities.md)                                      | Steps 2 & 5 — full inventory of what Fabius-Frames can do (24 sections). Scan the TOC during the brief, deep-dive specific sections during build |
...
| [techniques.md](../fabius-frames/references/techniques.md)                           | Steps 3 & 5 — 13 primitive animation techniques with code patterns (adapt, don't copy-paste)                                                   |
| [html-in-canvas-patterns.md](../fabius-frames/references/html-in-canvas-patterns.md) | Step 5 — complete code patterns for HTML-in-Canvas effects (lives in the fabius-frames skill)                                                    |
