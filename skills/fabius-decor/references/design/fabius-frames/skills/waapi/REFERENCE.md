---
name: fabius-decor-waapi
description: Web Animations API adapter patterns for Fabius-Frames. Use when authoring element.animate() motion, Animation currentTime seeking, document.getAnimations(), KeyframeEffect timing, fill modes, or native browser animations that must render deterministically in Fabius-Frames.
---

# Web Animations API for Fabius-Frames

Fabius-Frames can seek Web Animations API animations through its `waapi` runtime adapter. WAAPI is useful when you want native browser keyframes with JavaScript-created timing and no GSAP dependency.
...
The adapter calls `document.getAnimations()`, sets each animation's `currentTime` to Fabius-Frames time in milliseconds, then pauses it.
...
- Simple timelines representable as keyframes, delays, and offsets.
...
- Assuming clip-local start time is automatic. The WAAPI adapter seeks document-level animation time; model clip offsets with `delay` or create the animation on an element whose visibility is controlled by Fabius-Frames timing.
...
- Fabius-Frames adapter source: `packages/core/src/runtime/adapters/waapi.ts`
