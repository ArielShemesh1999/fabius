---
name: fabius-decor-three
description: Three.js and WebGL adapter patterns for Fabius-Frames. Use when creating deterministic Three.js scenes, WebGL canvas layers, AnimationMixer timelines, camera motion, shader-driven visuals, or canvas renders that respond to Fabius-Frames hf-seek events.
---

# Three.js for Fabius-Frames

Fabius-Frames supports Three.js through its `three` runtime adapter. The adapter does not own your scene. It publishes Fabius-Frames time and dispatches a seek event so your composition can render the exact frame.

## Contract

- Create the scene, camera, renderer, materials, and assets synchronously when possible.
- Render from Fabius-Frames time, not wall-clock time.
...
- Fabius-Frames adapter source: `packages/core/src/runtime/adapters/three.ts`
