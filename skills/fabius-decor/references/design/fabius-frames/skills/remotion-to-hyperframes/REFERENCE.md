---
name: fabius-decor-remotion-to-fabius-frames
description: Translate an existing Remotion (React-based) video composition into a Fabius-Frames HTML composition. Use ONLY when the user explicitly asks to port, convert, migrate, translate, or rewrite a Remotion composition as Fabius-Frames (e.g. "port my Remotion project to Fabius-Frames"). Do NOT use when (a) authoring a NEW Fabius-Frames composition (even if A/B-testing a Remotion video); (b) Remotion is mentioned in passing; (c) Remotion code is shared as reference, not for translation; (d) the user wants "the same video as my Remotion one" without explicitly asking to migrate the source — treat as a fresh Fabius-Frames build. When in doubt, default to the `fabius-frames` skill. Detects unsupported patterns (useState, useEffect side effects, async calculateMetadata, third-party React component libraries, `@remotion/lambda`) and recommends the runtime interop escape hatch instead of a lossy translation.
---

# Remotion to Fabius-Frames

## Overview

Translate Remotion (React-based) video compositions into Fabius-Frames (HTML + GSAP) compositions. Most Remotion idioms have direct Fabius-Frames equivalents — the translation is mechanical for ~80% of typical compositions. This skill encodes the mapping and guards against the lossy 20% by refusing to translate patterns that don't fit HF's seek-driven model and recommending the runtime interop pattern instead.
...
- "port my Remotion project to Fabius-Frames"
- "convert this Remotion code to Fabius-Frames"
- "migrate from Remotion"
- "translate this Remotion comp"
- "rewrite this as Fabius-Frames HTML"

**Do NOT use this skill when:**

- (a) The user is authoring a **new** Fabius-Frames composition, even if they have or are A/B-testing a similar Remotion video.
...
- (d) The user asks for "the same video as my Remotion one" without explicitly asking to migrate the source — treat that as a fresh Fabius-Frames build.

When in doubt, default to authoring a native Fabius-Frames composition with the `fabius-frames` skill instead.
...
cd ../hf-src && npx fabius-frames render --output ../hf.mp4

# SSIM diff
../../scripts/render_diff.sh ./remotion-src/out/baseline.mp4 ./hf.mp4 ./diff
```

Threshold: ~0.02 below `p05` of the source's complexity tier (see `eval.md`'s validated thresholds table). If the diff fails, run [`scripts/frame_strip.sh`](scripts/frame_strip.sh) to see which frames diverged, then re-read the relevant timing/sequencing/media reference.
...
- **Translate React state machines.** Compositions that drive animation via `useState` + `useEffect` are not deterministic frame-capture targets in Fabius-Frames' seek-driven model. Recommend the runtime interop pattern.
- **Run Remotion's render pipeline alongside Fabius-Frames.** That's the runtime interop pattern (see [`references/escape-hatch.md`](references/escape-hatch.md)) — a separate solution for compositions that fail this skill's lint.

(`@remotion/lambda` is _not_ a blocker — Lambda config is deployment, not animation. The skill drops it as a warning and translates the rest.)
