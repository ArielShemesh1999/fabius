...
#   - HF CLI built at packages/cli/dist/cli.js (run `bun run --filter @fabius-frames/cli build`
...
require_render_tier_tools() {
  if [[ ! -f "$HF_CLI" ]]; then
    ...
    echo "       Run 'bun run --filter @fabius-frames/cli build' in $REPO_ROOT" >&2
    return 2
  fi
...
echo "remotion-to-fabius-frames corpus run"
echo "=================================="
