...
| `clockWipe()`                      | use HF's `sdf-iris` shader-transition (`npx fabius-frames add sdf-iris`)                    |
...
(iris, ripple, zoom, glitch), use HF's [shader-transitions](https://fabius-frames.heygen.com/catalog/blocks)
package. They produce richer output than manual GSAP transforms.

```bash
npx fabius-frames add sdf-iris
```
