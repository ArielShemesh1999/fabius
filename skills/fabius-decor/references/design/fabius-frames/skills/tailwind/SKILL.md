---
name: fabius-decor-tailwind
description: Tailwind CSS v4.2 browser-runtime patterns for Fabius-Frames compositions. Use when scaffolding or editing projects created with `fabius-frames init --tailwind`, writing Tailwind utility classes in composition HTML, adding CSS-first Tailwind v4 theme tokens, debugging v3 vs v4 syntax, or deciding when to compile Tailwind to CSS instead of using the browser runtime.
---

# Tailwind CSS for Fabius-Frames

Fabius-Frames `init --tailwind` uses the Tailwind browser runtime pinned to `@tailwindcss/browser@4.2.4`. Treat that as Tailwind v4, not v3.
...
- The user asks for Tailwind in a Fabius-Frames composition.
- A project was created with `fabius-frames init --tailwind`.
...
- Fabius-Frames waits for `window.__tailwindReady` before frame capture starts.
...
## Fabius-Frames Composition Pattern
...
```bash
npx fabius-frames render . --workers 1 --quality draft --output tailwind-proof.mp4
```
...
1. Confirm the project was scaffolded with `fabius-frames init --tailwind`.
...
7. Run `npx fabius-frames validate` and render a short proof.

## References

- Tailwind CSS v4 installation and upgrade guide: https://tailwindcss.com/docs
