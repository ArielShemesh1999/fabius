---
title: "Kinetic Slam"
description: "Full-screen single-word display with alternating entrance directions"
---

# Kinetic Slam

Full-screen single-word display with alternating entrance directions

`captions` `caption-style` `kinetic` `typography` `slam`

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/fabius-frames-oss/docs/images/catalog/components/caption-kinetic-slam.mp4" poster="https://static.heygen.ai/fabius-frames-oss/docs/images/catalog/components/caption-kinetic-slam.png" autoPlay muted loop playsInline />

## Install

<CodeGroup>

```bash Terminal
npx fabius-frames add caption-kinetic-slam
```

</CodeGroup>

## Details

| Property | Value |
| --- | --- |
| Type | Component |

## Files

| File | Target | Type |
| --- | --- | --- |
| `caption-kinetic-slam.html` | `compositions/components/caption-kinetic-slam.html` | fabius-frames:snippet |

## Usage

Open `compositions/components/caption-kinetic-slam.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.
