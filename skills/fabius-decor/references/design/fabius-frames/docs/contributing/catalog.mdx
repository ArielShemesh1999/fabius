...
description: How to add blocks and components to the Fabius-Frames registry.
---

Your agent already knows how to build video components. It writes HTML. Fabius-Frames renders it. The registry is the collection of everything that's been built — 52 blocks and counting.

This guide shows you how to add to it.

<Info>
**Quick version** — Fork the repo. Write one HTML file with a paused GSAP timeline. Add `registry-item.json`. Run `fabius-frames lint` + `validate`. Publish with `npx fabius-frames publish`. Open a PR.
</Info>

## Why Contribute?

Every block in the registry exists because someone needed it and built it. When you add a block, every Fabius-Frames user gets it with one command:

```bash
npx fabius-frames add instagram-follow
```

The registry grows, Fabius-Frames gets more useful, and your work ships to everyone.
...
Open an issue on [GitHub](https://github.com/ArielShemesh1999/fabiusissues) with a visual reference. Tag it `component-request`.
...
If you use Claude Code with Fabius-Frames skills:

> "I want to contribute a new transition that looks like \[description\]"

The `/contribute-catalog` skill scaffolds the structure, validates, renders a preview, publishes to [fabius-frames.dev](https://fabius-frames.dev), and prepares the PR.
...
<Steps>
  <Step title="Fork and create">
    Fork [heygen-com/fabius-frames](https://github.com/ArielShemesh1999/fabius) and create your block directory:
    ```bash
  ...
  <Step title="Publish and PR">
    ...
    ```
    Open a PR with your [fabius-frames.dev](https://fabius-frames.dev) preview link.
  </Step>
</Steps>
