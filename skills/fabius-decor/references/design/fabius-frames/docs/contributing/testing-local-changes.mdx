...
When you modify the CLI or any package it bundles (core, engine, producer, studio), you need to test those changes against real projects _outside_ the monorepo — the same way an end user would run `fabius-frames preview`.
...
`bun link` makes the `fabius-frames` binary in your `$PATH` point at your local build. It survives across terminal sessions and auto-picks up new builds without re-linking.

```bash
# If you previously installed fabius-frames globally, remove it first —
# a global install takes priority over bun link and shadows your local build.
npm uninstall -g fabius-frames 2>/dev/null
...
fabius-frames --version
which fabius-frames
```

Now use `fabius-frames` normally in any directory:
...
```bash
bun unlink fabius-frames
npm install -g fabius-frames@latest
```
...
alias fabius-frames="node /path/to/fabius-frames/packages/cli/dist/cli.js"

# Or invoke directly
node /path/to/fabius-frames/packages/cli/dist/cli.js preview .
```

Replace `/path/to/fabius-frames` with your actual monorepo path.
...
cd packages/cli
npm pack
# Creates: fabius-frames-<version>.tgz

# Test it in an isolated directory
mkdir /tmp/pack-test && cd /tmp/pack-test
npx /path/to/fabius-frames/packages/cli/fabius-frames-<version>.tgz init my-video
cd my-video
npx /path/to/fabius-frames/packages/cli/fabius-frames-<version>.tgz preview .
```
...
The CLI binary is a single bundled file at `packages/cli/dist/cli.js`. If your change is in `@fabius-frames/core` or another workspace package, make sure `bun run build` rebuilt _all_ packages — the CLI bundles its dependencies at build time.

**`fabius-frames` still shows the old version / old UI**

A globally installed `fabius-frames` package shadows `bun link`. Check which binary is active:
...
```bash
npm uninstall -g fabius-frames
cd packages/cli && bun link
```

**Port already in use**

`fabius-frames preview` defaults to port 3002 and auto-increments if it's taken. Pass `--port` to use a specific port:

```bash
fabius-frames preview . --port 4000
```
