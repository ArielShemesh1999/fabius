...
<script>
  document.getElementById("hero").src = __fabius-frames.getVariables().heroVideo;
</script>
```
...
<video id="hero" data-start="0" data-track-index="0"></video>
<script>
  const { heroVideo, heroDuration } = __fabius-frames.getVariables();
...
| Frame rate | `--fps` flag on `fabius-frames render`, or `config.fps` in the SDK |
...
Inside any composition script, call `window.__fabius-frames.getVariables()` to get the resolved variable values. The return type is `Partial<Record<string, unknown>>` — use destructuring with defaults matching the declared `default` values:
...
<html data-composition-variables='[
  ...
  <body>
    <div data-composition-id="card" data-width="1920" data-height="1080">
      ...
      <script>
        const { title = "Untitled", color = "#111827" } = __fabius-frames.getVariables();
...
`__fabius-frames.getVariables()` is a shorthand for `window.__fabius-frames.getVariables()` and works in both top-level and sub-composition scripts. The runtime automatically scopes sub-compositions so each instance sees its own resolved values.
...
```bash Terminal
# Inline JSON
npx fabius-frames render --variables '{"title":"Q4 Report","color":"#1d4ed8"}' --output q4.mp4

# JSON file
npx fabius-frames render --variables-file ./vars.json --output out.mp4

# Fail on undeclared or mistyped variables
npx fabius-frames render --variables '{"title":"Q4 Report"}' --strict-variables --output out.mp4
```
...
| CLI `--variables` flag | Highest | `fabius-frames render --variables '{...}'` |
...
If you are building tooling on top of `@fabius-frames/core`, the variable declarations are readable without rendering:
