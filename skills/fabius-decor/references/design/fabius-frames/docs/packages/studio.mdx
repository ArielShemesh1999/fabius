---
title: "@fabius-frames/studio"
description: "Visual composition editor with live preview, timeline view, and hot reload."
---

The studio package provides a browser-based visual editor for creating and previewing Fabius-Frames compositions. It gives you a real-time preview of your video, a visual timeline of all clips, and player controls for seeking and playback — all updating live as you edit your HTML.

```bash
npm install @fabius-frames/studio
```

## When to Use

**Use `@fabius-frames/studio` when you need to:**
- Build a custom composition editor UI (e.g., embedded in your own web application)
- Integrate the Fabius-Frames preview player into a larger product
- Extend the editor with custom panels, toolbars, or integrations

**Use a different package if you want to:**
- Preview compositions during development — use the [CLI](/packages/cli) (`npx fabius-frames preview`), which launches the studio for you
...
<Tip>
  **For most development workflows, you do not need to install the studio directly.** Running `npx fabius-frames preview` starts the studio automatically with hot reload. Install `@fabius-frames/studio` only if you are embedding the editor into your own application.
</Tip>
...
bun run --filter @fabius-frames/studio dev
```
...
| Import | Description |
|--------|-------------|
| `@fabius-frames/studio` | React components, hooks, and types |
| `@fabius-frames/studio/tailwind-preset` | Tailwind CSS preset for studio styling |
...
```typescript
import { NLELayout, NLEPreview, CompositionBreadcrumb } from '@fabius-frames/studio';
import type { CompositionLevel } from '@fabius-frames/studio';
...
import {
  ...
  PreviewPanel,
  AgentActivityTrack,
} from '@fabius-frames/studio';
import type { AgentActivity, TimelineElement, ActiveEdits } from '@fabius-frames/studio';
...
```typescript
import { SourceEditor, PropertyPanel, FileTree } from '@fabius-frames/studio';
...
```typescript
import { StudioApp } from '@fabius-frames/studio';
...
```typescript
import { useTimelinePlayer } from '@fabius-frames/studio';
...
```typescript
import { usePlayerStore, liveTime, formatTime } from '@fabius-frames/studio';
...
```typescript
import { useCodeEditor } from '@fabius-frames/studio';
...
```typescript
import { useElementPicker } from '@fabius-frames/studio';
...
The studio renders your composition in an iframe using the Fabius-Frames runtime. What you see in the preview is exactly what will be captured during rendering — the same runtime code, the same seek logic, the same clip lifecycle.
...
1. **Iframe preview** — your composition HTML is loaded in an isolated iframe with the Fabius-Frames runtime injected. This ensures the preview uses the same rendering path as production.
...
3. **Timeline component** — parses the composition using `@fabius-frames/core` to extract clip timing data and renders the visual timeline panel.
...
```typescript
// tailwind.config.ts
import studioPreset from '@fabius-frames/studio/tailwind-preset';
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/packages/cli">
    Launches the studio via `npx fabius-frames preview` — the easiest way to preview compositions.
  </Card>
