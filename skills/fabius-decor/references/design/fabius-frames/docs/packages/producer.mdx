...
```bash
npm install @fabius-frames/producer
```

## When to Use

**Use `@fabius-frames/producer` when you need to:**
...
- Render from the command line without writing code — use the [CLI](/packages/cli) (`npx fabius-frames render`)
...
<Steps>
  ...
  <Step title="Inject the Fabius-Frames runtime">
...
```typescript
import { createRenderJob, executeRenderJob } from '@fabius-frames/producer';
...
```typescript
import { createRenderJob } from '@fabius-frames/producer';
...
```typescript
import type { ProgressCallback, RenderStatus } from '@fabius-frames/producer';
...
```typescript
import { RenderCancelledError } from '@fabius-frames/producer';
...
```typescript
import { startServer } from '@fabius-frames/producer/server';
...
```typescript
import { createRenderHandlers, createProducerApp } from '@fabius-frames/producer/server';
...
<Info>
  Docker mode requires Docker to be installed and running. Run `npx fabius-frames doctor` to verify your environment. See [Deterministic Rendering](/concepts/determinism) for details on what makes Docker mode deterministic.
</Info>
...
When GPU encoding is enabled, Fabius-Frames detects the available FFmpeg hardware encoder automatically. To check your system's capabilities:
...
```typescript
import { resolveConfig } from '@fabius-frames/producer';
