...
The `fabius-frames` CLI is the primary way to work with Fabius-Frames. It handles project creation, live preview, rendering, linting, and diagnostics — all from your terminal.

```bash
npm install -g fabius-frames
# or use directly with npx
npx fabius-frames <command>
```
...
<Tip>
  The CLI is the recommended starting point for all Fabius-Frames users. It wraps the producer, engine, and studio packages so you do not need to install them separately.
</Tip>
...
<Tabs>
  <Tab title="Agent mode (default)">
    ...
    npx fabius-frames init my-video --example blank --video video.mp4
    npx fabius-frames render --output output.mp4 --fps 30 --quality standard
    npx fabius-frames upgrade --check --json
    ```
  </Tab>
  <Tab title="Human mode">
    ...
    npx fabius-frames catalog --human-friendly
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Create a project">
    ...
    ```bash
    npx fabius-frames init --example warm-grain
    ```
    You will be prompted for a project name, or pass it as an argument:
    ```bash
    npx fabius-frames init my-video --example warm-grain
    ```
  ...
  <Step title="Preview in browser">
    ...
    ```
    The Fabius-Frames Studio opens in your browser. Edit `index.html` and the preview updates instantly.
  </Step>
  ...
  <Step title="Render to MP4">
    ...
    ```bash
    npx fabius-frames render --output output.mp4
    ```
    Render a specific composition instead of `index.html`:
    ```bash
    npx fabius-frames render -c compositions/intro.html -o intro.mp4
    ```
    For deterministic output, add `--docker`:
    ```bash
    npx fabius-frames render --docker --output output.mp4
    ```
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Create">
    ...
    npx fabius-frames init my-video --example blank --tailwind
    ...
    npx fabius-frames add claude-code-window

    # Add a component (effect / snippet)
    npx fabius-frames add shader-wipe

    # Target a different project dir
    npx fabius-frames add shader-wipe --dir ./my-video

    # Headless / CI (skip clipboard; also: --json for a machine-readable result)
    npx fabius-frames add shader-wipe --no-clipboard --json
    ```
    ...
    `add` reads [`fabius-frames.json`](#fabius-frames-json) at the project root to know which registry to pull from and where to drop files. If the file is missing but the directory looks like a Fabius-Frames project (has `index.html`), a default `fabius-frames.json` is written the first time you run `add`.
    ...
    Trying `add` with an example's name (e.g. `fabius-frames add warm-grain`) emits a clear error pointing you at `init --example`.
    ...
    npx fabius-frames catalog

    # Filter by type or tag
    npx fabius-frames catalog --type block
    npx fabius-frames catalog --type block --tag social

    # Machine-readable JSON
    npx fabius-frames catalog --json
    ...
    ```bash
    npx fabius-frames compositions
    ```
    ...
    npx fabius-frames transcribe video.mp4 --model medium.en --language en

    # Import existing transcripts from other tools
    npx fabius-frames transcribe subtitles.srt
    npx fabius-frames transcribe captions.vtt
    ...
    **Supported transcript formats:**
    ...
    | Format | Source |
    |--------|--------|
    | whisper.cpp JSON | `fabius-frames init --video`, `fabius-frames transcribe` |
    ...
    npx fabius-frames tts "Welcome to Fabius-Frames"

    # Choose a voice
    npx fabius-frames tts "Hello world" --voice am_adam

    # Save to a specific file
    npx fabius-frames tts "Intro" --voice bf_emma --output narration.wav

    # Adjust speech speed
    npx fabius-frames tts "Slow and clear" --speed 0.8
    ...
    npx fabius-frames tts "Bonjour le monde" --voice af_heart --lang fr-fr

    # Read text from a file
    npx fabius-frames tts script.txt

    # List available voices
    npx fabius-frames tts --list
    ```
    ...
    npx fabius-frames remove-background avatar.mp4 -o transparent.webm

    # ProRes 4444 .mov for editing round-trip
    npx fabius-frames remove-background avatar.mp4 -o transparent.mov

    # Single image → transparent PNG
    npx fabius-frames remove-background portrait.jpg -o cutout.png

    # Layer separation: cutout AND inverse-alpha background plate in one pass
    npx fabius-frames remove-background avatar.mp4 \
      -o subject.webm --background-output plate.webm

    # Force CPU on a machine that has CoreML or CUDA
    npx fabius-frames remove-background avatar.mp4 -o transparent.webm --device cpu

    # Inspect detected providers without rendering
    npx fabius-frames remove-background --info
    ```
    ...
    The model is `u2net_human_seg` (MIT, ~168 MB ONNX). Weights download to `~/.cache/fabius-frames/background-removal/models/` on first run and are reused thereafter. Peak inference RAM is ~1.5 GB.
    ...
    ```bash
    npx fabius-frames capture https://stripe.com
    npx fabius-frames capture https://linear.app -o captures/linear
    npx fabius-frames capture https://example.com --json
    ```
    ...
    Output is a self-contained directory with a `CLAUDE.md` file that any AI agent can read to understand the captured site. Used by the `/website-to-fabius-frames` skill as step 1 of the video production pipeline.
  ...
  <Tab title="Preview">
    ...
    ```bash
    npx fabius-frames preview [dir]
    npx fabius-frames preview --port 4567
    ```
    ...
    Opens your composition in the Fabius-Frames Studio with live preview. Edits to `index.html` and any referenced sub-compositions are reflected automatically. The preview uses the same Fabius-Frames runtime as production rendering, so what you see is what you get.
    ...
    2. **Local studio mode** — if `@fabius-frames/studio` is installed in your project's `node_modules`, spawns Vite with full HMR for faster iteration.
    3. **Monorepo mode** — if running from the Fabius-Frames source repo, spawns the studio dev server directly.

    ### `publish`

    Upload the project and get back a stable `fabius-frames.dev` URL:

    ```bash
    npx fabius-frames publish [dir]
    npx fabius-frames publish --yes
    ```
    ...
    `publish` zips the current project, uploads it to the Fabius-Frames publish backend, and prints a stable `fabius-frames.dev` URL for that stored project.

    The printed URL already includes the claim token, so opening it on `fabius-frames.dev` lets the intended user claim the uploaded project and continue editing in the web app.
    ...
    ```bash
    npx fabius-frames lint [dir]
    npx fabius-frames lint [dir] --verbose   # include info-level findings
    npx fabius-frames lint [dir] --json      # machine-readable JSON output
    ```
    ```
    ◆  Linting my-project/index.html
    ...
    **Severity levels:**
    ...
    ```bash
    npx fabius-frames inspect [dir]
    npx fabius-frames inspect [dir] --json
    npx fabius-frames inspect [dir] --samples 15
    npx fabius-frames inspect [dir] --at 1.5,4,7.25
    ```
    ...
    ```bash
    npx fabius-frames layout [dir] --json
    ```
    ...
    ```bash
    npx fabius-frames snapshot my-project --at 2.9,10.4,18.7
    npx fabius-frames snapshot my-project --frames 10
    ```
  ...
  <Tab title="Build">
    ...
    npx fabius-frames render --format webm --output overlay.webm

    # With options
    npx fabius-frames render --output output.mp4 --fps 60 --quality high

    # Opt out of local browser GPU capture
    npx fabius-frames render --no-browser-gpu --output cpu-browser.mp4

    # Add hardware FFmpeg encoding
    npx fabius-frames render --gpu --output gpu.mp4
    ```
    ...
    | `--variables` | JSON object | — | Variable overrides merged over `data-composition-variables` defaults. Read via `window.__fabius-frames.getVariables()` |
    ...
    <html
      ...
      <body>
        ...
        <script>
          const vars = window.__fabius-frames.getVariables();
    ...
    npx fabius-frames render --output default.mp4
    ...
    npx fabius-frames render --variables-file ./vars.json --output out.mp4
    ```
    ...
    npx fabius-frames render --format webm --output captions.webm
    ...
    ```bash
    npx fabius-frames benchmark [dir]
    ```
  ...
  <Tab title="Utilities">
    ...
    ```bash
    npx fabius-frames doctor
    ```
    ```
    fabius-frames doctor
    ...
    **CI gating.** `fabius-frames doctor --json` always exits 0 on successful execution — the command succeeded if it produced valid output. Whether the environment is healthy is carried in the `ok` field of the payload, so a new CLI release (which flips `Version.ok` to `false`) never breaks your pipeline. Pipe through `jq` to gate on the payload instead:
    ...
    ```bash
    npx fabius-frames upgrade
    npx fabius-frames upgrade --check         # check and exit (no prompt)
    npx fabius-frames upgrade --check --json  # machine-readable for agents
    npx fabius-frames upgrade --yes           # show upgrade commands without prompting
    ```
    ...
    npx fabius-frames browser ensure

    # Print the browser executable path (for scripting)
    npx fabius-frames browser path

    # Remove cached Chrome download
    npx fabius-frames browser clear
    ```

    The `path` subcommand outputs only the path, useful in scripts: `$(npx fabius-frames browser path)`.
    ...
    npx fabius-frames feedback --rating 5

    # Rating with optional details
    npx fabius-frames feedback --rating 3 --comment "render succeeded but GSAP timeline didn't animate"
    ```
    ...
    ```bash
    npx fabius-frames telemetry enable
    npx fabius-frames telemetry disable
    npx fabius-frames telemetry status
    ```
    ...
    Install Fabius-Frames skills for AI coding tools, including first-party runtime adapter skills:

    ```bash
    # Install to all default targets (Claude Code, Gemini CLI, Codex CLI)
    npx fabius-frames skills

    # Install to specific tools
    npx fabius-frames skills --claude
    npx fabius-frames skills --cursor
    npx fabius-frames skills --claude --gemini
    ```
    ...
    **Using `fabius-frames skills` is already fine** — as of v0.4.5 the CLI sets `GIT_CLONE_PROTECTION_ACTIVE=0` on the child environment, which is the opt-in knob Git provides for exactly this case. You don't need to do anything.

    **If you ran `npx skills add heygen-com/fabius-frames` directly** (bypassing the Fabius-Frames CLI), set the env var yourself:

    ```bash
    GIT_CLONE_PROTECTION_ACTIVE=0 npx skills add heygen-com/fabius-frames
    ```

    This is tracked in [GH #316](https://github.com/ArielShemesh1999/fabiusissues/316). An upstream fix in the `skills` CLI itself is the right long-term answer; until that lands, the env var is the correct workaround.
  </Tab>
</Tabs>
...
2. `HYPERFRAMES_API_KEY` environment variable (fabius-frames alias)
...
fabius-frames auth login --api-key

# Pipe a key from stdin (CI-friendly)
echo "$HEYGEN_API_KEY" | fabius-frames auth login --api-key
```
...
```bash
fabius-frames auth status
fabius-frames auth status --json   # machine-readable
```
...
```bash
fabius-frames auth logout
fabius-frames auth logout --keep-api-key   # clear only an OAuth session
fabius-frames auth logout --yes            # skip the confirmation prompt
```
...
Render a Fabius-Frames composition on HeyGen's hosted cloud — no local Chrome, no local ffmpeg, no AWS to manage. Sign in once with `fabius-frames auth login` and the same credential drives every `cloud` subcommand.

```bash
fabius-frames auth login                          # one-time
fabius-frames cloud render ./my-video             # zip + upload + poll + download
fabius-frames cloud render ./my-video --no-wait   # submit and exit with the render_id
fabius-frames cloud list                          # browse recent renders
```
...
End-to-end render: zips the project (excluding `.git`, `node_modules`, `dist`, `.next`, `coverage`, dotfiles), uploads it via `POST /v3/assets`, submits `POST /v3/fabius-frames/renders`, polls `GET /v3/fabius-frames/renders/{id}` until the render completes or fails, and streams the resulting video to disk.

Render parameters mirror the local `fabius-frames render` UX where they overlap:
...
fabius-frames cloud render --quality high --fps 60

# Fire-and-forget with a webhook (no local polling).
fabius-frames cloud render --callback-url https://example.com/hf-hook --no-wait

# Re-render an already-uploaded composition (skips zip + upload).
fabius-frames cloud render --asset-id asst_abc123

# Render from a public URL (no upload).
fabius-frames cloud render --url https://cdn.example.com/site.zip
```
...
```bash
fabius-frames cloud render . --idempotency-key "$(uuidgen)"
```
...
```bash
fabius-frames cloud list
fabius-frames cloud list --limit 50 --json
fabius-frames cloud list --all
```
...
```bash
fabius-frames cloud get hfr_abc123
fabius-frames cloud get hfr_abc123 --json
```
...
```bash
fabius-frames cloud delete hfr_abc123
fabius-frames cloud delete hfr_abc123 --no-confirm --json
```

### When to pick `cloud` vs `lambda` vs local render

- `fabius-frames render` (local): fastest iteration loop. Use during composition authoring.
- `fabius-frames lambda render`: bring-your-own-AWS distributed rendering. Use when you've already invested in AWS and want chunked parallelism on your own account.
- `fabius-frames cloud render`: zero-infra option. HeyGen runs the render; you pay per credit. Use when you don't want to manage Chrome/ffmpeg/AWS locally.

### Auth + base URL

`cloud` reuses the credential resolved by `fabius-frames auth status`. Override the API base for staging tests with `HEYGEN_API_URL` (default `https://api.heygen.com`).

## fabius-frames lambda

Deploy Fabius-Frames distributed rendering to AWS Lambda and drive renders from your laptop or CI.

The `fabius-frames lambda` command group wraps the `@fabius-frames/aws-lambda` SDK plus AWS SAM so an end-to-end render is three commands:

```bash
fabius-frames lambda deploy
fabius-frames lambda render ./my-project --width 1920 --height 1080 --wait
fabius-frames lambda destroy   # when you're done
```
...
Builds `packages/aws-lambda/dist/handler.zip` and SAM-deploys the stack at `examples/aws-lambda/template.yaml`. On success, writes `<cwd>/.fabius-frames/lambda-stack-<stackName>.json` so the other subcommands don't need to re-derive the bucket / state-machine ARN.

```bash
fabius-frames lambda deploy \
  --stack-name=fabius-frames-prod \
  --region=us-east-1 \
  --concurrency=8 \
  --memory=10240
```
...
```bash
fabius-frames lambda sites create ./my-project
# → siteId: abc1234deadbeef0  (stable across re-runs of the same tree)

fabius-frames lambda render ./my-project --site-id=abc1234deadbeef0 --width 1920 --height 1080
```
...
```bash
fabius-frames lambda render ./my-project \
...
The composition can be parameterised with `--variables` / `--variables-file`, mirroring the local `fabius-frames render` flags. Variables flow into the Step Functions execution input and reach every chunk worker as `window.__hfVariables`. Mismatches against the composition's `data-composition-variables` declaration print as warnings; pass `--strict-variables` to fail the command instead.

```bash
fabius-frames lambda render ./my-template --site-id=abc1234deadbeef0 \
...
```bash
fabius-frames lambda render-batch ./my-template \
  --batch ./users.jsonl \
  --width 1920 --height 1080 \
  --max-concurrent 10
```
...
Pass `--json` for the machine-readable form. Poll each execution via `fabius-frames lambda progress <renderId>` (or use the returned `executionArn`).
...
```bash
fabius-frames lambda render-batch ./my-template --batch ./users.jsonl \
...
```bash
fabius-frames lambda progress hf-render-abcd1234
```
...
fabius-frames lambda policies user

# Print { TrustRelationship, InlinePolicy } for an IAM role (default: cloudformation principal).
fabius-frames lambda policies role --principal=cloudformation

# Validate a checked-in policy still covers the CLI's needs.
fabius-frames lambda policies validate ./infra/iam/fabius-frames-deploy.json
```
...
`fabius-frames lambda` keeps per-stack metadata under `<cwd>/.fabius-frames/lambda-stack-<name>.json` so the verbs don't need to call `describe-stacks` every time. Commit the file to a repo or `.gitignore` it depending on your workflow — it contains the bucket name, state-machine ARN, and region, none of which are secrets but all of which are AWS-account-identifying.

## fabius-frames cloudrun

The Google Cloud counterpart to `fabius-frames lambda`. Deploys Fabius-Frames distributed rendering to Cloud Run + Cloud Workflows and drives renders from your laptop or CI. Wraps the `@fabius-frames/gcp-cloud-run` SDK plus `terraform` (the module shipped with the package) and `gcloud` / Cloud Build for the image.

```bash
fabius-frames cloudrun deploy --project my-gcp-project
fabius-frames cloudrun render ./my-project --width 1920 --height 1080 --wait
fabius-frames cloudrun destroy --project my-gcp-project   # when you're done
```
...
```bash
fabius-frames cloudrun deploy --project my-gcp-project --region us-central1
fabius-frames cloudrun deploy --project my-gcp-project --image us-central1-docker.pkg.dev/my-gcp-project/fabius-frames/fabius-frames-render:v1
```

Flags: `--project` (required), `--region` (default `us-central1`), `--image` (skip the build), `--repo` (Artifact Registry repo, default `fabius-frames`). Machine sizing / scaling: `--cpu` (1/2/4/8, default 4), `--memory` (e.g. `32Gi`, default `16Gi`), `--max-instances` (render fan-out ceiling, default 100), `--timeout` (per-request seconds, max 3600). Omitted sizing flags keep the module defaults; for anything finer, apply the Terraform module directly.
...
`fabius-frames cloudrun` caches the deployed stack's coordinates under `~/.fabius-frames/cloudrun-state.json` (project id, region, bucket, service URL, workflow id) so `render` / `progress` / `destroy` don't need them re-passed. None are secrets, but all are GCP-project-identifying.

## fabius-frames.json

`fabius-frames init` writes a `fabius-frames.json` file at the root of every new project. `fabius-frames add` reads it to know which registry to pull items from and where to drop them. Edit the file (or delete it to fall back to defaults) to reshape your project layout or point at a custom registry.
...
| Field | Description |
|-------|-------------|
| `registry` | Base URL of the registry `add` pulls from. Defaults to the public Fabius-Frames registry. |
...
<CardGroup cols={2}>
  ...
  <Card title="Studio" icon="palette" href="/packages/studio">
    The editor UI that powers `fabius-frames preview`. Use directly to embed in your own app.
  </Card>
