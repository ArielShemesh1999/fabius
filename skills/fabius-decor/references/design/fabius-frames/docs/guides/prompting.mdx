...
| -------------------------- | -------------------------------------------------------------------------- |
| `/fabius-frames`             | Composition authoring — HTML structure, timing, captions, TTS, transitions |
| `/fabius-frames-cli`         | Dev-loop CLI — `init`, `lint`, `inspect`, `preview`, `render`, `doctor`    |
| `/fabius-frames-media`       | Asset preprocessing — `tts`, `transcribe`, `remove-background`             |
| `/fabius-frames-registry`    | Block and component installation via `fabius-frames add`                     |
| `/website-to-fabius-frames`  | Capture a URL and turn it into a video; runs the full [Fabius-Frames pipeline](/guides/pipeline) |
| `/gsap`                    | GSAP animation API — timelines, easing, ScrollTrigger, plugins             |

<Tip>
  Always prefix Fabius-Frames prompts with `/fabius-frames` (or invoke the skill another way for non-Claude agents). This loads the skill context explicitly so the agent gets composition rules right the first time, instead of relying on whatever it remembers about web video.
</Tip>

## Claude Design

Claude Design uses a different setup. Download [`claude-design-fabius-frames.md`](https://github.com/ArielShemesh1999/fabiusblob/main/docs/guides/claude-design-fabius-frames.md) from GitHub (click the ↓ button), then **attach it to your chat** (don't paste the URL — file attachments produce better output):
...
Claude Design produces a valid first draft (brand identity, scene content, animations, transitions). Download the ZIP and refine in any AI coding agent with `npx fabius-frames preview` running. See the [Claude Design guide](/guides/claude-design) for the full workflow.

## The two prompt shapes

Most successful Fabius-Frames prompts fall into one of two shapes.
...
> Using `/fabius-frames`, create a 10-second product intro with a fade-in title over a dark background and subtle background music.

> Make a 9:16 TikTok-style hook video about [topic] using `/fabius-frames`, with bouncy captions synced to a TTS narration.
...
You give the agent something to work with — a URL, a doc, a CSV, a transcript — and ask it to synthesize that into a video. This is where Fabius-Frames shines because the agent does the research/summarization step *and* the production step in one flow.

> Take a look at this GitHub repo https://github.com/ArielShemesh1999/fabius and explain its uses and architecture to me using `/fabius-frames`.

> Summarize the attached PDF into a 45-second pitch video using `/fabius-frames`.

> Read this changelog and turn the top three changes into a 30-second release announcement video using `/fabius-frames`.

> Turn this CSV into an animated bar chart race using `/fabius-frames`.
...
Fabius-Frames is a conversation. After the first render, talk to the agent the way you'd talk to a video editor — don't re-prompt from scratch:
...
- **Don't ask for React / Vue components.** Fabius-Frames compositions are plain HTML with `data-*` attributes and a GSAP timeline. Asking for "a React component for the intro" forces the agent to translate later.
...
- **Don't skip the slash command.** Without `/fabius-frames`, the agent may guess at HTML video conventions instead of using the framework's actual rules (`class="clip"` on timed elements, `window.__timelines` registration, etc.).
- **Don't paste long error logs into the prompt without context.** Run `npx fabius-frames lint` and `npx fabius-frames validate` first — lint catches structural issues, validate catches runtime errors (JS exceptions, missing assets, contrast problems).
...
1. `npx fabius-frames init my-video` — scaffold a project (skills install automatically)
2. Open the project in Claude Code (or Cursor / Codex)
3. Prompt with `/fabius-frames` and one of the shapes above
4. `npx fabius-frames preview` — watch in the browser as the agent edits
5. Iterate with small targeted prompts
6. `npx fabius-frames render --output final.mp4` when you're happy
