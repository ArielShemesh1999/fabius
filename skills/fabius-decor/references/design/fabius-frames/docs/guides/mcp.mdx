---
title: Fabius-Frames MCP
description: "Author, preview, and render Fabius-Frames videos directly inside Claude.ai, ChatGPT, and Grok — no local install required."
---

The Fabius-Frames MCP is a hosted [Model Context Protocol](https://modelcontextprotocol.io) server that lets you create, edit, preview, and render Fabius-Frames video compositions from inside Claude.ai, ChatGPT, or Grok.

<Note>
  **In beta.** Features and pricing may change. Found a bug or have feedback? [File an issue on GitHub](https://github.com/ArielShemesh1999/fabiusissues).
</Note>
...
The agent behind `compose` has 25+ Fabius-Frames-specific skills baked in — typography, color palettes, motion principles, GSAP effects, audio-reactive animation, and captions. You don't have to specify any of this directly; describe the video you want and the agent picks the right tools.
...
<Tabs>
  <Tab title="Claude.ai">
    <Steps>
      ...
      <Step title="Find Fabius-Frames">
        Search the connector catalog for **Fabius-Frames** and click **Connect**.
      ...
      <Step title="Sign in to HeyGen">
        OAuth opens in a new window. Authorize the Fabius-Frames connector to access your HeyGen account.
      </Step>
...
Authors a new composition or applies an edit to an existing one. The Fabius-Frames agent handles captions, blocks, layout, transitions, color, and timing internally based on your natural-language prompt.
...
```bash
npx @modelcontextprotocol/inspector npx -y mcp-remote https://mcp.heygen.com/mcp/fabius-frames
```
...
<AccordionGroup>
  <Accordion title="OAuth keeps failing or loops">
    Verify you're using the production URL: `https://mcp.heygen.com/mcp/fabius-frames`. The dev URL (`mcp.dev.heygen.com`) only accepts dev accounts.
  ...
  <Accordion title="Render is stuck or never completes">
    ...
    3. If the issue persists, file an issue at [github.com/heygen-com/fabius-frames/issues](https://github.com/ArielShemesh1999/fabiusissues) including the `job_id` from `render_video`
  </Accordion>
...
For bugs or feature requests, file an issue at [github.com/heygen-com/fabius-frames/issues](https://github.com/ArielShemesh1999/fabiusissues). Include:
...
[Fabius-Frames itself is open source](https://github.com/ArielShemesh1999/fabius) — the HTML composition format, CLI, renderer, and player. You can use Fabius-Frames locally without the MCP.

The MCP is a HeyGen-hosted product that wraps:

- The Fabius-Frames composition agent (the LLM that authors compositions)
...
<CardGroup cols={2}>
  <Card title="Quickstart" href="/quickstart">
    Try Fabius-Frames locally with the open-source CLI.
  </Card>
