...
description: "Fast reference for video editors and creative people directing agents, cutting timing, tweaking layouts, previewing, and publishing Fabius-Frames projects."
---

Use this as a fast reference when you are directing agents, cutting timing, making visual layout tweaks, previewing, and sharing Fabius-Frames projects.

## The Fast Loop

```bash
npx fabius-frames init my-video --example blank
cd my-video
...
npx fabius-frames render --quality standard --output review.mp4
```

For fast iteration renders, use draft quality:

```bash
npx fabius-frames render --quality draft --output draft.mp4
```

For final delivery:

```bash
npx fabius-frames render --quality high --fps 30 --output final.mp4
```
...
Common Fabius-Frames project folders:
...
Run Fabius-Frames commands from the project root, where `index.html` lives. If you are not sure where you are, run `pwd` then `ls`. If you see `index.html`, you are in the right place.
...
| Command | Use it for |
| --- | --- |
| `npx fabius-frames init my-video` | Create a new project |
| `npx fabius-frames init my-video --example warm-grain` | Start from a visual template |
| `npx fabius-frames init my-video --video source.mp4` | Import video and generate captions from the source audio |
| `npx fabius-frames capture https://example.com` | Capture a website as source material for a video |
| `npx fabius-frames preview` | Open the live Studio preview |
| `npx fabius-frames lint` | Catch structural mistakes before preview or render |
| `npx fabius-frames validate` | Run the composition in headless Chrome to catch runtime errors |
| `npx fabius-frames inspect` | Find text overflow and layout problems across the timeline |
| `npx fabius-frames snapshot --at 1,3,5` | Save PNG checks at exact timestamps |
| `npx fabius-frames render --output final.mp4` | Render the video |
| `npx fabius-frames publish` | Upload the project and get a shareable Fabius-Frames URL |
| `npx fabius-frames doctor` | Check Node.js, FFmpeg, Chrome, Docker, and other dependencies |
| `npx fabius-frames docs` | Open local CLI docs |
| `npx fabius-frames upgrade` | Check for a newer CLI version |
...
| Goal | Command |
| --- | --- |
| Fast iteration | `npx fabius-frames render --quality draft --output draft.mp4` |
| Review link | `npx fabius-frames render --quality standard --output review.mp4` |
| Final export | `npx fabius-frames render --quality high --fps 30 --output final.mp4` |
| Transparent overlay | `npx fabius-frames render --format webm --output overlay.webm` |
| Deterministic output | `npx fabius-frames render --docker --output final.mp4` |
...
Publish zips the current project, uploads it, and prints a stable `fabius-frames.dev` URL. The URL includes a claim token so the recipient can open it, claim the project, and continue editing in the web app.

```bash
npx fabius-frames publish ./my-video   # publish a specific folder
npx fabius-frames publish --yes        # skip the confirmation prompt in scripts
```
...
Use it when you want the agent to open the Fabius-Frames Studio preview, play or scrub the video, click timeline controls, inspect visible UI text, capture screenshots for review, or record proof of a tested flow.

For editor-facing changes, keep `npx fabius-frames preview` running, then have the agent use `agent-browser` against the local preview URL.

## Quick Fixes

| Problem | Command or check |
| --- | --- |
| Preview will not start | `npx fabius-frames doctor` |
| Port already in use | `npx fabius-frames preview --port 4567` |
| Render fails | `npx fabius-frames lint` then `npx fabius-frames validate` |
| Need exact frame checks | `npx fabius-frames snapshot --at 1,2.5,5` |
| Text overflows in the frame | `npx fabius-frames inspect` |
| Final render is too slow | Try `--quality draft`, reduce image sizes, or lower `--fps` |
| Need to share editable project | `npx fabius-frames publish` |
