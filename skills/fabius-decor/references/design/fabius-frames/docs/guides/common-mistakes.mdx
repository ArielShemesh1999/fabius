...
description: "Pitfalls that break Fabius-Frames compositions."
---

These are mistakes that cannot be caught by the linter. For automated checks, run `npx fabius-frames lint` (see [CLI](/packages/cli#lint)).
...
<AccordionGroup>
  ...
  <Accordion title="Composition duration shorter than video">
    ...
    **After (fixed):**
    ...
    <Tip>
      A quick check: run `npx fabius-frames compositions` to see the resolved duration of each composition. If it is shorter than expected, your timeline needs extending.
    </Tip>
  </Accordion>
  ...
  <Accordion title="Missing class='clip' on timed elements">
    ...
    **After (fixed):**
    ...
    <Note>
      The linter catches this one: `npx fabius-frames lint` will flag timed elements missing `class="clip"`.
    </Note>
  </Accordion>
  ...
  <Accordion title="Expected HDR output but got SDR">
    ...
    **Cause:** By default, Fabius-Frames only switches to HDR encoding when a source `<video>` or `<img>` is tagged with BT.2020 / PQ / HLG color metadata. Common reasons HDR is not engaged:
    ...
    2. **Wrong output format.** HDR output requires MP4. `--format mov` and `--format webm` fall back to SDR — Fabius-Frames logs a warning when this happens.
...
1. **Run the linter:** `npx fabius-frames lint` — catches most structural issues
