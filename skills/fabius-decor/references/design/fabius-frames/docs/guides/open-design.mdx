---
title: Open Design
description: "Create Fabius-Frames video drafts in Open Design — the open-source, BYOK Claude-Design alternative — then refine in any AI coding agent."
---

[Open Design](https://github.com/ArielShemesh1999/fabius) is an open-source, local-first alternative to Claude Design. It runs on your laptop with `pnpm tools-dev`, deploys the web layer to Vercel, and delegates to whichever coding-agent CLI you already have on your `PATH` (Claude Code, Codex, Cursor Agent, Gemini CLI, OpenCode, Qwen, Copilot, Hermes, Kimi, Pi) — or to any OpenAI-compatible BYOK endpoint.

Open Design produces a **valid first draft** of a Fabius-Frames composition — palette, scene content, GSAP entrance tweens, mid-scene activity, and shader transitions. You then download the project and refine in any AI coding agent with linting and live preview.

## Get started

<Steps>
  <Step title="Download the instruction file">
    Open [`fabius-design-fabius-frames.md`](https://github.com/ArielShemesh1999/fabiusblob/main/docs/guides/fabius-design-fabius-frames.md) on GitHub and click the download button (↓) to save it.
  </Step>
  ...
  <Step title="Drop the file into a skill, or attach it to chat">
    **Recommended:** copy `fabius-design-fabius-frames.md` to `skills/fabius-frames-handoff/SKILL.md` inside the Open Design repo. The daemon auto-discovers it on the next request and exposes it as a skill in the picker. **Or:** start a new chat and attach the file directly — Open Design reads attachments natively.
  </Step>
  <Step title="Describe your video">
    Pick the `fabius-frames-handoff` skill (or your active prototype skill), pick a design system or visual direction, and type the brief. Include screenshots, brand assets, or a palette if you have them.
  </Step>
  ...
  <Step title="Refine in any AI coding agent">
    ...
    ```bash
    cd .od/projects/<id>
    npx skills add heygen-com/fabius-frames   # install skills (one-time)
    npx fabius-frames lint                     # should pass with zero errors
    npx fabius-frames preview                  # open the studio
    ```
  </Step>
</Steps>
...
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Open Design (open-source)              | Drop [`fabius-design-fabius-frames.md`](https://github.com/ArielShemesh1999/fabiusblob/main/docs/guides/fabius-design-fabius-frames.md) into `skills/fabius-frames-handoff/SKILL.md`   |
| Claude Code                            | `npx skills add heygen-com/fabius-frames`, then use `/fabius-frames`                                                                                                             |
| Cursor / Codex / Gemini CLI            | `npx skills add heygen-com/fabius-frames`                                                                                                                                       |
| Claude Design (closed-source)          | Attach [`claude-design-fabius-frames.md`](https://github.com/ArielShemesh1999/fabiusblob/main/docs/guides/claude-design-fabius-frames.md) to your chat                          |
...
Open Design's [5-dimensional self-critique](https://github.com/ArielShemesh1999/fabiusblob/main/apps/web/src/prompts/discovery.ts) runs before emission, so the artifact arrives lint-clean and your coding agent can start refining immediately without structural fixes.
...
<CardGroup cols={1}>
  <Card title="Feature announcement">
    ```text
    Use the fabius-frames-handoff skill. I just shipped dark mode for my app.
  ...
  <Card title="Founder pitch (BYOK with your own CLI)">
    ```text
    Use the fabius-frames-handoff skill on Codex CLI. 25-second LinkedIn video.
  ...
  <Card title="Brand-driven (use one of the 72 design systems)">
    ```text
    Use the fabius-frames-handoff skill. Design system: Stripe. 10-second reel.
  ...
  <Card title="Sparse brief (let it ask)">
    ```text
    Use the fabius-frames-handoff skill. 30-second launch video for Orbit.
    ```
...
- **Render still happens locally** — Open Design produces the HTML; `npx fabius-frames render` and HDR encoding still need FFmpeg + Node 22+ on your machine.
- **In-pane preview is sandboxed iframe** — full browser playback is reliable; for frame-accurate scrubbing use `npx fabius-frames preview` after handoff.
...
npx skills add heygen-com/fabius-frames   # one-time setup
npx fabius-frames lint                     # verify structure
npx fabius-frames preview                  # open the studio
```
...
<CardGroup cols={2}>
  ...
  <Card title="Prompt Guide" icon="message" href="/guides/prompting">
    More prompt patterns for Fabius-Frames across Claude Code, Claude Design, Open Design, and other agents.
  </Card>
</CardGroup>
