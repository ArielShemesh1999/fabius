...
description: "Solutions for common Fabius-Frames issues."
---
...
<AccordionGroup>
  <Accordion title='"No composition found"'>
    ...
    **Fix:** Run `npx fabius-frames init` to create a composition from an [example](/examples), or verify your `index.html` has the correct structure:
  ...
  <Accordion title='"FFmpeg not found"'>
    ...
    After installing, run `npx fabius-frames doctor` to verify the CLI can find it.

    <Tip>
      If you cannot install FFmpeg, use [Docker mode](/guides/rendering) instead — it bundles FFmpeg inside the container: `npx fabius-frames render --docker --output output.mp4`
    </Tip>
  </Accordion>

  <Accordion title="Lint errors">
    Run `npx fabius-frames lint` to check for common structural issues (see [CLI: lint](/packages/cli#lint)):
  ...
  <Accordion title="Preview not updating">
    ...
    2. Stop and restart `npx fabius-frames preview`
  ...
  <Accordion title="Preview stutters or plays at a low frame rate">
    ...
    **Temporary workaround:** render to mp4 and watch the output. Render is accurate regardless of per-frame cost.
    ...
    ```bash Terminal
    npx fabius-frames render --quality draft --output preview.mp4
    ```
  ...
  <Accordion title="Docker mode fails to start">
    ...
    - **Image pull fails:** Check your internet connection; the first render downloads the Fabius-Frames Docker image
  </Accordion>
  ...
  <Accordion title="Render is slow">
    ...
    2. Run `npx fabius-frames benchmark` to find the optimal worker count for your system
...
Run `npx fabius-frames doctor` to check your environment:
...
1. Run `npx fabius-frames info` to gather system and project details
2. Check [GitHub Issues](https://github.com/ArielShemesh1999/fabiusissues) for similar reports
3. Open a new issue with the output of `npx fabius-frames info` and steps to reproduce
