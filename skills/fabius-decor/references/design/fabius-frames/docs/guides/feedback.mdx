...
description: "How Fabius-Frames collects feedback, what data is collected, and how to opt out."
---

Fabius-Frames occasionally asks how a render went or how a Studio session felt. This page explains why we do it, when prompts appear, what data is collected, and how to disable them.
...
After a successful `fabius-frames render`, a short prompt may appear:
...
HYPERFRAMES_FEEDBACK_INTERVAL=5 fabius-frames render --output out.mp4
```
...
**Note:** Disabling CLI telemetry (`fabius-frames telemetry disable`) does not suppress the Studio bar — Studio feedback is gated separately. The submitted data is still sent through the same anonymous PostHog pipeline, so the same privacy guarantees apply.

### `fabius-frames feedback` command

You can submit feedback manually at any time:

```bash
# Quick rating
fabius-frames feedback --rating 5

# Rating with details
fabius-frames feedback --rating 3 --comment "render succeeded but GSAP timeline didn't animate text overlay"
```
...
This command collects a doctor summary automatically, flushes telemetry, and exits. It appears under the **Settings** group in `fabius-frames --help`.

## Agent Runtimes

When an AI agent is detected, Fabius-Frames **skips the interactive readline prompt** and prints a structured hint instead:

```
  [fabius-frames] Agent feedback: fabius-frames feedback --rating <1-5> --comment "..."
```

Agents can then submit feedback using the `fabius-frames feedback` command above.
...
The `doctor_summary` is a compact string with environment context — included automatically so you don't need to run `fabius-frames doctor` when reporting a problem:
...
The CLI persists feedback state in `~/.fabius-frames/config.json`:
...
Studio stores the equivalent state in `localStorage` under the `fabius-frames-studio:` prefix.
...
```bash
# Via CLI command (persisted to ~/.fabius-frames/config.json)
fabius-frames telemetry disable

# Or via environment variable (per-session)
HYPERFRAMES_NO_TELEMETRY=1 fabius-frames render --output out.mp4

# Or via DO_NOT_TRACK (respects the global standard)
DO_NOT_TRACK=1 fabius-frames render --output out.mp4
```

Once disabled, the `fabius-frames feedback` command will print `Telemetry is disabled. Feedback not sent.` and exit without sending anything.
...
fabius-frames render --quiet --output out.mp4
```
...
- [`fabius-frames feedback`](/packages/cli#feedback) — CLI command reference
