...
When the video is grounded in an existing source (a website, a brand site, a competitor reference), start with capture. Fabius-Frames ships a built-in capture command for websites:

```bash
npx fabius-frames capture https://example.com -o my-video/capture
```
...
`DESIGN.md` is also the input format for [Open Design](/guides/fabius-design) and [Claude Design](/guides/claude-design); both produce a `DESIGN.md` you can drop into a Fabius-Frames project.
...
| Techniques       | 2-3 picks from the [techniques library](https://github.com/ArielShemesh1999/fabiusblob/main/skills/fabius-frames/references/techniques.md): SVG path drawing, Canvas 2D, CSS 3D, per-word typography, Lottie, video compositing, typing effects, variable fonts, MotionPath, velocity transitions, audio-reactive |
...
Fabius-Frames ships multiple TTS adapters (Kokoro, ElevenLabs, HeyGen); see [`/fabius-frames-media`](/guides/prompting) for the skill that picks one. After generating audio, update `STORYBOARD.md` with the real beat boundaries from `transcript.json`.
...
After each composition is built, run a self-review for layout, asset placement, and animation quality. The [`/fabius-frames`](/guides/prompting) skill encodes the composition rules: required `class="clip"` attributes, GSAP timeline registration, `data-*` attribute semantics, and adapter registries.
...
```bash
npx fabius-frames lint                              # static HTML structure checks
npx fabius-frames validate                          # loads in headless Chrome, catches runtime errors
npx fabius-frames snapshot my-video --at 2.9,10.4   # PNGs at beat midpoints
```
...
The pipeline delivers the localhost Studio URL as the handoff. Your AI agent runs `npx fabius-frames preview` and shares the project URL. Rendering to MP4 is on-demand:

```bash
npx fabius-frames render --output my-video.mp4
```
...
- For surgical tweaks, open a composition file directly (e.g. `compositions/beat-3-proof.html`) and adjust animations, colors, or layout. `npx fabius-frames preview` shows changes live.
...
- Capturing a website with the [/website-to-fabius-frames](/guides/prompting) skill, which follows it end-to-end.
...
- Learning Fabius-Frames, because the artifacts leave every creative decision inspectable on disk.
