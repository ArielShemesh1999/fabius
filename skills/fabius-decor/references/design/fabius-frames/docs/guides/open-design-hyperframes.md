---
name: fabius-frames-handoff
description: |
  Produce a Fabius-Frames-valid HTML composition — paused GSAP timeline, data
  attributes, scene structure — that any AI coding agent can immediately
  refine with `npx fabius-frames lint` and `npx fabius-frames preview`. Use when
  the brief mentions "video", "reel", "motion graphic", "title card",
  "animated explainer", or pairs Open Design with Fabius-Frames for export.
triggers:
  - "fabius-frames"
  - "video"
  - "reel"
...
od:
  ...
  example_prompt: "Design a 15-second Instagram reel announcing dark mode for Taskflow (#6C5CE7). Output as a Fabius-Frames composition I can render locally."
---
...
3. **Any AI coding agent** (Claude Code, Codex, Cursor, …) — `npx fabius-frames
   lint`, `npx fabius-frames preview`, then iterate timing, eases, shader
   choices, pacing
...
- Appropriate shader choices for the mood (use the catalog at
  [fabius-frames.heygen.com/catalog](https://fabius-frames.heygen.com/catalog))
...
These are Fabius-Frames-structural and non-negotiable. Open Design's
...
3. **Scenes are children of `#stage`.** Each scene is `<div class="scene
   ...
   - `data-track-index="0"` (Fabius-Frames uses tracks for layering; visual
...
4. **GSAP timeline registered paused.** A single timeline created with
   ...
   Fabius-Frames engine drives the playhead.
...
7. **Shader transitions ONLY at scene boundaries**, and at most 2–3 in the
   whole video. Use Fabius-Frames' built-in shader blocks
...
Use Fabius-Frames' built-in shader blocks at scene boundaries. Pick by mood:
...
<head>
  ...
  <style>
    ...
    .scene.clip { /* Fabius-Frames toggles visibility per playhead */ }
...
<script>
  ...
  // Forward Fabius-Frames preview tokens (frame=, paused=, …) into the iframe
...
```bash
npx fabius-frames lint        # should pass with zero errors
npx fabius-frames preview     # opens the studio
npx fabius-frames render      # writes MP4
```

---

## Anti-AI-slop blacklist (Fabius-Frames-specific)
...
- **No wall-clock JS animations** — `setTimeout`, `setInterval`,
  ...
  Lottie are supported via [Fabius-Frames' Frame Adapter](https://fabius-frames.heygen.com/concepts/frame-adapters)
...
- The `fabius-frames add` registry (50+ blocks and components)
