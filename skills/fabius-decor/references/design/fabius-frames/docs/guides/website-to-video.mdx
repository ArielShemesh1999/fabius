...
<Steps>
  <Step title="Install skills">
    Skills teach your AI agent how to capture websites and create Fabius-Frames compositions. Install once — they persist across sessions.
...
<Note>
  You don't need to run `npx fabius-frames capture` manually — the skill instructs the agent to capture as the first step. The capture command is documented [below](#capture-command) for advanced use.
</Note>

## How the Pipeline Works

The skill follows the [Fabius-Frames pipeline](/guides/pipeline): seven steps, each producing a named artifact that feeds the next.
...
See [the pipeline guide](/guides/pipeline) for a detailed walkthrough of each step, the contents of every generated file, and how to iterate without re-running the whole pipeline. The structure is useful for any Fabius-Frames project, not just website captures.
...
<AccordionGroup>
  <Accordion title="Capture times out">
    ...
    ```bash
    npx fabius-frames capture https://example.com --timeout 180000
    ```
  </Accordion>
  ...
  <Accordion title="Agent doesn't find the skill">
    ...
    Lead your prompt with _"Use the /website-to-fabius-frames skill"_ for the most reliable results. Agents also discover it automatically when they see a URL and a video request.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Quickstart" icon="rocket" href="/quickstart">
    New to Fabius-Frames? Start here.
  </Card>
