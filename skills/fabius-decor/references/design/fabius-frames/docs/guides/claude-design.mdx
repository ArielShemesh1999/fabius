---
title: Claude Design
description: "Create Fabius-Frames video drafts in Claude Design, then refine in any AI coding agent."
---

Claude Design produces a **valid first draft** of a Fabius-Frames video — brand identity, scene content, layout, animations, and transitions. You then download the ZIP and refine in any AI coding agent (Claude Code, Cursor, Codex, Windsurf, etc.) with linting and live preview.

## Get started

<Steps>
  <Step title="Download the instruction file">
    Open [`claude-design-fabius-frames.md`](https://github.com/ArielShemesh1999/fabiusblob/main/docs/guides/claude-design-fabius-frames.md) on GitHub and click the download button (↓) to save it.
  </Step>
  ...
  <Step title="Attach the file + describe your video">
    Drag the `claude-design-fabius-frames.md` file into the chat. Describe what you want — include screenshots, brand assets, or a palette if you have them.
  </Step>
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Claude Design               | Download [`claude-design-fabius-frames.md`](https://github.com/ArielShemesh1999/fabiusblob/main/docs/guides/claude-design-fabius-frames.md) from GitHub and attach to your chat |
...
This template-first approach means the output passes `npx fabius-frames lint` with zero errors on first download — your coding agent can start refining immediately without structural fixes.
...
- **In-pane preview** — scrubbing is unreliable in Claude Design's iframe sandbox. Download and use `npx fabius-frames preview` locally for reliable playback.
- **No linting** — Claude Design can't run `npx fabius-frames lint`. The template-first skeletons handle structural validity, but the self-review checklist is the only QA before download.
...
<CardGroup cols={2}>
  <Card title="Prompt Guide" icon="message" href="/guides/prompting">
    More prompt patterns for Fabius-Frames across Claude Code, Claude Design, and other agents.
  </Card>
  <Card title="@fabius-frames/player" icon="play" href="/packages/player">
