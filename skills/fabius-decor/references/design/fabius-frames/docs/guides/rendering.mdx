...
Render your Fabius-Frames [compositions](/concepts/compositions) to MP4, MOV, WebM, GIF, or PNG sequences with the [CLI](/packages/cli). The rendering pipeline is frame-by-frame and seek-driven — see [Deterministic Rendering](/concepts/determinism) for how this works under the hood.
...
npx fabius-frames render --crf 15 --output pristine.mp4

# Target a specific bitrate (useful for size-constrained delivery)
npx fabius-frames render --video-bitrate 10M --output controlled.mp4
```
...
```bash Terminal
npx fabius-frames render --format gif --fps 15 --gif-loop 0 --output demo.gif
```

GIF output uses a two-pass FFmpeg palette encode (`palettegen` with diff statistics, then `paletteuse` with Sierra dithering) for better gradients and text edges than a single-pass conversion. GIFs are still much larger than MP4/WebM at the same dimensions, so prefer `--fps 15` and short compositions. Fabius-Frames caps GIF renders at 30fps.
...
Fabius-Frames has two separate GPU acceleration surfaces:
...
npx fabius-frames render --gpu --output encoded-fast.mp4

# Opt out of hardware Chrome/WebGL capture
npx fabius-frames render --no-browser-gpu --output software-browser.mp4
...
By default, Fabius-Frames uses **half of your CPU cores, capped at 4**:
...
npx fabius-frames render --workers 1 --output output.mp4

# Let Fabius-Frames pick based on your CPU
npx fabius-frames render --workers auto --output output.mp4

# Maximum parallelism (use with caution on laptops)
npx fabius-frames render --workers 8 --output output.mp4
```
...
```bash Terminal
# CLI flag
npx fabius-frames render --max-concurrent-renders 2 --output output.mp4
...
Fabius-Frames supports rendering with a transparent background — useful for overlays, lower thirds, subscribe cards, and any element you want to composite over other footage in a video editor.

### Recommended format: MOV (ProRes 4444)

```bash Terminal
npx fabius-frames render --format mov --output overlay.mov
```
...
```bash Terminal
npx fabius-frames render --format png-sequence --output frames/
```
...
When you render with `--format mov`, `--format webm`, or `--format png-sequence`, Fabius-Frames:
...
- Use `npx fabius-frames benchmark` to find optimal settings for your system
