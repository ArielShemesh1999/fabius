...
Fabius-Frames can render to HDR10 MP4 (H.265 10-bit, BT.2020) when your composition references HDR video or HDR still images. HDR is auto-detected by default from your media sources and falls back to SDR when none are present.

<Note>
  By default, Fabius-Frames probes your media and enables HDR only when HDR sources are present. Use `--hdr` to force HDR even without HDR sources, or `--sdr` to force SDR even when HDR sources are present.
</Note>
...
<Steps>
  <Step title="Add an HDR source to your composition">
    Fabius-Frames detects HDR from the source's color space metadata. The most reliable HDR sources are:
  ...
  <Step title="Render normally">
    ...
    HDR output requires `--format mp4`. If Fabius-Frames detects HDR sources, it renders HDR automatically. If you also pass `--format mov` or `--format webm`, Fabius-Frames logs a warning and falls back to SDR.
  </Step>
...
Fabius-Frames recognizes HDR video from its `ffprobe` color space metadata:
...
A valid HDR source is any MP4 whose stream metadata reports BT.2020 primaries plus PQ or HLG transfer. Fabius-Frames will detect it automatically:
...
Fabius-Frames supports HDR still images delivered as **16-bit PNGs** tagged with BT.2020 primaries and PQ transfer. Drop them into the composition as a normal `<img>`:
...
If HDR is enabled and you also pass `--format mov`, `--format webm`, or `--format gif`, Fabius-Frames logs a message and produces the equivalent SDR render. There is no error — the render still completes — so check the logs (or your verification step) to confirm you got HDR.
...
- **Player support** — the [`<fabius-frames-player>`](/packages/player) web component plays back the encoded MP4 in the browser and inherits whatever HDR support the host browser provides; it does not implement its own HDR pipeline
...
<CardGroup cols={2}>
  ...
  <Card title="Engine: HDR APIs" icon="gear" href="/packages/engine#hdr-apis">
    Public HDR utilities exported from `@fabius-frames/engine`
  </Card>
