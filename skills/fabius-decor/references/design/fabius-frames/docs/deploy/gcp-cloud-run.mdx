...
description: "Deploy distributed Fabius-Frames rendering to Google Cloud Run + Cloud Workflows, and drive renders from a laptop or CI."
---

Fabius-Frames ships a Google Cloud deployment that mirrors the [AWS Lambda](/deploy/aws-lambda) one: a single Cloud Run service fronts a Cloud Workflows definition that fans renders out across many parallel chunk workers, with intermediate artifacts in Google Cloud Storage. The render primitives are identical — only the storage, compute, and orchestration adapters differ.

It's the right choice for teams already running their backend and storage on Google Cloud who want distributed Fabius-Frames rendering without adding AWS infrastructure.
...
gcloud builds submit . \
  --tag us-central1-docker.pkg.dev/PROJECT/fabius-frames/fabius-frames-render:v1

# 2. Apply the module.
cd node_modules/@fabius-frames/gcp-cloud-run/terraform
terraform init
terraform apply \
  -var project_id=PROJECT \
  -var region=us-central1 \
  -var image=us-central1-docker.pkg.dev/PROJECT/fabius-frames/fabius-frames-render:v1
```
...
import {
  renderToCloudRun,
  getRenderProgress,
} from "@fabius-frames/gcp-cloud-run/sdk";

const handle = await renderToCloudRun({
  projectDir: "./my-composition",
  config: { fps: 30, width: 1920, height: 1080, format: "mp4" },
  bucketName: "fabius-frames-render-my-project",
  projectId: "my-project",
  location: "us-central1",
  workflowId: "fabius-frames-render",
  serviceUrl: "https://fabius-frames-render-abc.us-central1.run.app",
});
