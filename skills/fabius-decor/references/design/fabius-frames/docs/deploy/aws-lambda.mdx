---
title: AWS Lambda
description: "Deploy distributed Fabius-Frames rendering to AWS Lambda and drive renders from a laptop or CI."
---

Fabius-Frames ships a first-class AWS Lambda deployment: one Lambda function fronts a Step Functions standard workflow that fans renders out across many parallel chunk workers, with intermediate artifacts in S3. End-to-end is three commands once your AWS credentials are configured.

```bash
fabius-frames lambda deploy
fabius-frames lambda render ./my-project --width 1920 --height 1080 --wait
fabius-frames lambda destroy
```
...
│   handler.mjs                                                    │
│     ├─ Action="plan"        → @fabius-frames/producer/distributed  │
│     ├─ Action="renderChunk" → @fabius-frames/producer/distributed  │
│     └─ Action="assemble"    → @fabius-frames/producer/distributed  │
...
The Lambda handler is a thin dispatch: parse the Step Functions event, download inputs from S3 into `/tmp`, call the OSS primitive from `@fabius-frames/producer/distributed`, upload outputs back, return a small JSON result. Everything heavy — capture, encode, audio mix — happens inside the OSS primitives.
...
| AWS SAM CLI | `fabius-frames lambda deploy/destroy` shells out to `sam deploy`/`sam delete`. | [Install guide](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/install-sam-cli.html) |
| `bun` | Used to build `packages/aws-lambda/dist/handler.zip` at deploy time. | `npm install -g bun` or [bun.sh](https://bun.sh) |
| Fabius-Frames repo checkout | `lambda deploy` builds the Lambda handler ZIP from source. Adopters who deploy outside a checkout can set `HYPERFRAMES_REPO_ROOT` to point at one. | `git clone https://github.com/ArielShemesh1999/fabius` |

## Three deployment paths

### Path 1 — `fabius-frames lambda` CLI (recommended)

The CLI is a thin wrapper around the SAM template + the `@fabius-frames/aws-lambda` SDK. For most adopters this is the right starting point.
...
The `--wait` flag blocks and streams per-chunk progress + accrued cost; drop it to fire-and-forget, then poll with `fabius-frames lambda progress <renderId>` on your own cadence.

See the [CLI reference](/packages/cli#fabius-frames-lambda) for full flag documentation.
...
fabius-frames lambda render ./my-project --site-id=a1b2c3d4e5f6g7h8 \
...
For users already running CDK, the `@fabius-frames/aws-lambda` package exports a `Fabius-FramesRenderStack` L2 construct that emits the same topology as the SAM template:

```ts
import { App, CfnOutput, Stack } from "aws-cdk-lib";
import { Fabius-FramesRenderStack } from "@fabius-frames/aws-lambda/cdk";

const app = new App();
const stack = new Stack(app, "MyApp");
const render = new Fabius-FramesRenderStack(stack, "Render", {
  projectName: "fabius-frames",
...
`aws-cdk-lib` and `constructs` are declared as **optional peer dependencies** of `@fabius-frames/aws-lambda`, so consumers who only need the SDK don't pay the CDK import cost.
...
Lambda renders are billed by GB-seconds (Lambda billed duration × configured memory) plus a tiny per-state-transition fee for Step Functions standard workflows. `fabius-frames lambda progress` exposes the running tally:

```bash
fabius-frames lambda progress my-render-id
# Status:    SUCCEEDED
# Progress:  100%
# Frames:    480 / 480
# Lambdas:   5
# Cost:      $0.0214 (Lambda $0.0210 + SFN $0.0004)
# Output:    s3://fabius-frames-renders/.../output.mp4
```
...
The IAM credential running `lambda deploy` doesn't have permission to create the service role CloudFormation needs. Run `fabius-frames lambda policies user` and attach the printed policy to your IAM user (or take the `policies role` output and have your admin create a deploy role).

### `Lambda function failed: PLAN_HASH_MISMATCH`

Step Functions invoked a `renderChunk` with a plan hash that didn't match the planDir on S3. Almost always means the producer version differs between the local `plan()` build and the deployed Lambda ZIP. Re-run `fabius-frames lambda deploy` (which rebuilds the ZIP) and re-render.
...
fabius-frames lambda deploy --stack-name=<your-stack>
```
...
The render bucket is created with CloudFormation `Retain` on delete — `fabius-frames lambda destroy` (or `sam delete`) tears the function + state machine down but the bucket survives. This is intentional: it protects final-rendered MP4s from being lost when you re-deploy. To fully reclaim storage, empty + delete the bucket via the AWS console / `aws s3 rb`.

## What's NOT in the v1 surface

- **Webhooks on completion.** Not in v1 — poll with `fabius-frames lambda progress` or watch the Step Functions execution. A `--webhook` flag with an SNS topic is on the Phase 6c backlog.
