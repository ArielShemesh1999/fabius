...
If your typed-data payload genuinely exceeds 256 KiB (e.g. a long structured record per render with no media), [file an issue](https://github.com/shear559/fabiusissues/new) — there's a clean path via S3-hosted variable files, but we want to see real demand before designing the API.
...
If your template pipeline hits a wall the docs don't cover, [file an issue on GitHub](https://github.com/shear559/fabiusissues/new) — the batch surface is new and the feedback loop on it is short.
