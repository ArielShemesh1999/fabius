...
Fabius-Frames templates are compositions that take typed variables — a name, a colour, a chart payload, a CTA URL — and produce a finished render parameterised by those values. Pair a template with the deployed Lambda stack and `lambda render-batch`, and you get personalised-video-at-scale in one CLI call:
...
This guide walks the full loop: declare variables on a composition, iterate locally with `fabius-frames render`, deploy to Lambda once, then fan out N renders from a batch file. The same flow also drives single personalised renders via `lambda render --variables` and programmatic batches via `renderToLambda({ variables })`.

```mermaid
flowchart LR
  A["Local iteration<br/>fabius-frames render --variables"] --> B["Deploy stack<br/>fabius-frames lambda deploy"]
  B --> C["Upload site once<br/>fabius-frames lambda sites create"]
  C --> D["Fan out renders<br/>fabius-frames lambda render-batch"]
  D --> E["N personalised videos<br/>in S3"]
```

## What's a template

A template is just a Fabius-Frames composition whose top-level HTML element declares a `data-composition-variables` attribute listing the variables it accepts. The composition reads the runtime values via `window.__fabius-frames.getVariables()`.
...
<body style="margin:0;background:#f6f5f1">
  ...
  <script>
    (function () {
      var v = window.__fabius-frames.getVariables();
...
The runtime helper is exposed as a global — `window.__fabius-frames.getVariables()` — not as a fetchable module. Use a plain `<script>` (not `<script type="module">`) so the runtime is already initialized when the script executes.
...
Fast iteration is the whole point of templates — you should not have to deploy to Lambda to see how a value looks. Use `fabius-frames render` locally with `--variables` (or `--variables-file`) to render the template against any payload:

```bash
fabius-frames render --variables '{"title":"Hello Alice","accentColor":"#ff0000"}' \
...
```bash
fabius-frames render --variables-file ./alice.json --strict-variables \
  --output renders/alice-preview.mp4
```

## Deploying to Lambda

Templates render on the standard `fabius-frames lambda` stack — there's no special template-only deployment. Run:
...
```bash
fabius-frames lambda sites create ./my-template
...
```bash
fabius-frames lambda render ./my-template \
  --site-id abc1234deadbeef0 \
  --width 1920 --height 1080 \
...
`--wait` streams progress lines until the render finishes; without it the CLI returns immediately and you poll via `fabius-frames lambda progress <renderId>`.
...
```bash
fabius-frames lambda progress arn:aws:states:us-east-1:1234:execution:hf:hf-render-abcd
```
...
The same surface is available from TypeScript via `@fabius-frames/aws-lambda/sdk`. Deploy the site once and parallel-render the batch:

```typescript
import { deploySite, renderToLambda } from "@fabius-frames/aws-lambda/sdk";
...
https://fabius-frames.heygen.com/deploy/templates-on-lambda#working-with-large-variables
...
If your typed-data payload genuinely exceeds 256 KiB (e.g. a long structured record per render with no media), [file an issue](https://github.com/ArielShemesh1999/fabiusissues/new) — there's a clean path via S3-hosted variable files, but we want to see real demand before designing the API.
...
In-process vs distributed crossover: for a single render under ~30 seconds, the in-process renderer (`fabius-frames render`) wins on latency because there's no S3 round-trip per chunk. Distributed wins for renders over ~60 seconds or when you need a personalised batch — that's the whole reason this surface exists. (The Phase 7 small-render shortcut, when it lands, will collapse the gap for short renders.)

## Migrating from @remotion/lambda inputProps

Remotion's `inputProps` API and Fabius-Frames' `variables` are isomorphic — both are JSON objects injected as render-time overrides on top of declared composition defaults. The mapping is mechanical:

| Remotion | Fabius-Frames |
|----------|-------------|
| `Composition.defaultProps` | `data-composition-variables` declaration on the root HTML element |
| `useCurrentFrame()` + `props.<x>` | `window.__fabius-frames.getVariables().<x>` (read once on DOMContentLoaded) |
...
- **TypeScript types generated from `data-composition-variables`**: `fabius-frames types generate <projectDir>` is sketched and may land in v1.5; it would let SDK callers `import type { Variables } from "./template/variables"` for autocomplete + typecheck.
...
If your template pipeline hits a wall the docs don't cover, [file an issue on GitHub](https://github.com/ArielShemesh1999/fabiusissues/new) — the batch surface is new and the feedback loop on it is short.
