...
**Don't forget to update test fixtures:** Add your locale code to `EXPECTED_LOCALES` in `apps/web/tests/i18n/locales.test.ts` and add a `LOCALE_LABEL` assertion (e.g., `expect(LOCALE_LABEL.it).toBe('Italiano');`). Run `pnpm --filter @fabius-design/web test` to verify.
...
9. **Run checks:**
   ...
   pnpm --filter @fabius-design/web test  # Covers locale/content drift tests
   ```
