---
name: fabius-decor-pptx-html-fidelity-audit
description: Audit a python-pptx export against its source HTML deck, identify layout/content drift (footer overflow, cropped content, missing italic/em, lost styling, off-rhythm spacing), and re-export with strict footer-rail + cursor-flow layout discipline.
triggers:
  - "pptx fidelity"
  - "pptx audit"
  - "ppt 跑掉"
  ...
  - "verify pptx"
  - "html to pptx"
  - "compare ppt with html"
  - "fix the pptx"
  - "ppt is cut off"
  - "italic missing in pptx"
  - "re-export the deck"
  - "fidelity audit"
---
...
The audit is five steps. Don't skip any — the discipline only works if the audit produces a real list of issues to drive the re-export. A fix-without-audit pass tends to leave half the issues alive.
...
- `scripts/extract_pptx.py` — dump every shape on every slide as JSON. Run before the audit. Also run on the *original* export to compare, and on the *re-exported* one to confirm.
...
- The deck has slide types beyond what this skill covers (multi-column dashboards, embedded images, charts) → `layout-discipline.md`.
...
An earlier approach leaned on visual diffing — render the .pptx through Keynote → PDF → PNG, screenshot the HTML through Chrome headless, stitch them side-by-side with `magick`. It worked, but with three sharp drawbacks:
