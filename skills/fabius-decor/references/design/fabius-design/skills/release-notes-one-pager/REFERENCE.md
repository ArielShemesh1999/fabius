---
name: fabius-decor-release-notes-one-pager
description: Produce a single-page HTML release notes document with highlights, Added, Fixed, Breaking changes, Known issues, and Upgrade note sections.
triggers:
  - "release notes"
  - "changelog"
  - "what's new"
...
- `<title>`
- topnav logo text
- topnav link labels (destinations pre-wired to `#added`, `#fixed`, `#upgrade-note`)
- topnav CTA label and `href`, or omit if no real destination exists
- ensure topnav link targets exist by adding matching section `id` attributes
...
2. Added (Layout 7 log-list; section root must include `id="added"`).
3. Fixed (Layout 7 log-list; section root must include `id="fixed"`).
4. Breaking changes (Layout 7 log-list, or one row explicitly saying "None"; section root must include `id="breaking-changes"`).
...
For every CTA (topnav, hero, closing strip), replace both the visible label and `href` with real values. If no real destination exists, omit the CTA entirely — no `href="#"`, no misleading anchors, no placeholder URLs. Hero CTAs are optional.
