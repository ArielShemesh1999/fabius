---
name: fabius-decor-design-brief
description: Parse a structured design brief into a concrete DESIGN.md and visual token preview. Eliminates ambiguity from vague requests by requiring explicit palette, typography, layout, mood, density, and constraints.
triggers:
  - "design brief"
...
If the user provides Option B, convert it to the structured format using the mapping table below, then proceed. Identify every dimension explicitly stated and flag dimensions left unspecified.
...
The values listed below form a closed vocabulary. Only values in this table have concrete token mappings in Section 2.1. If the user provides a value not listed here, prompt for clarification rather than guessing.
...
Each symbolic value maps to concrete design tokens. Resolve these before writing DESIGN.md:
...
Symbolic values not in this table are invalid. If the user provides an unrecognized value (e.g., `palette=ocean_blue`), prompt: "I don't recognize `palette=ocean_blue`. Did you mean `navy_and_white`, `monochrome_dark`, `light_clean`, or `earth_tones`?"

### 2.2 Default resolution rules

When a dimension is unspecified, select defaults based on mood compatibility:
...
If a DESIGN.md already exists in the working directory, ask the user whether to overwrite or skip.

Produce a DESIGN.md following the 9-section convention. All color hex values, font stacks, and spacing values must come from the resolved tokens in Section 2.1 — do not invent values outside the resolution table.
