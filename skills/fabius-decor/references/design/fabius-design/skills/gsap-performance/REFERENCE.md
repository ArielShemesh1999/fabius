---
name: fabius-decor-gsap-performance-gsap-performance
description: GSAP performance — prefer transforms, avoid layout thrashing, will-change, batching, quickTo. Use when optimizing animations, reducing jank, or targeting smooth 60fps.
triggers:
  - "gsap performance"
  - "animation jank"
  - "60fps animation"
...
Apply when optimizing GSAP animations for smooth 60fps, reducing layout/paint cost, or when the user asks about performance, jank, or fast animation best practices.
...
Hint the browser to promote a layer on elements that will animate:

```css
will-change: transform;
```

Only set on elements that are actually animating — not on everything "just in case."

## Batch Reads and Writes

GSAP batches updates internally. When mixing GSAP with direct DOM reads/writes, avoid interleaving reads and writes in a way that causes repeated layout thrashing. Do all reads first, then all writes.
...
Prefer **gsap.quickTo()** for properties updated on every event. It reuses a single tween instead of creating a new one on each call.
...
- Avoid animating large numbers of properties on many elements at once; simplify or sequence if needed.
...
- ❌ Set **will-change** or **force3D** on every element; use only on elements that are actually animating.
- ❌ Create hundreds of overlapping tweens or ScrollTriggers without testing on low-end devices.
- ❌ Ignore cleanup; stray tweens and ScrollTriggers keep running and hurt performance.
