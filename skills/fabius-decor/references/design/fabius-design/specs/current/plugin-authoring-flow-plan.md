...
- R3. The task should create a real plugin folder containing at minimum `SKILL.md` and `fabius-design.json`, with optional examples/assets when the user asks for them.
...
- No new plugin spec dialect. The generated folder must validate against the existing Open Design plugin shape in `docs/plugins-spec.md` and `docs/schemas/fabius-design.plugin.v1.json`.
...
- [x] U3. **Plugin authoring scenario**
  ...
  **Files:**
  - Create: `plugins/_official/scenarios/od-plugin-authoring/fabius-design.json`
  ...
  **Approach:**
  - The scenario should instruct the agent to output a folder such as `generated-plugin/` with `SKILL.md`, `fabius-design.json`, and optional `examples/` or `assets/`.
...
- [x] U4. **Generated folder visibility in project output**
  ...
  **Approach:**
  - Preserve relative paths so `generated-plugin/fabius-design.json` and `generated-plugin/SKILL.md` remain visibly grouped.
  ...
  **Test scenarios:**
  - Happy path: a project with `generated-plugin/fabius-design.json` and `generated-plugin/SKILL.md` renders a folder group.
...
- fabius-design.json with valid metadata, mode, task kind, inputs, and any pipeline/context references
