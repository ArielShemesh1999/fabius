...
> **For agentic workers:** REQUIRED SUB-SKILL: Use the discipline set:subagent-driven-development (recommended) or the discipline set:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
```bash
cd /c/Users/ekada/OneDrive/Desktop/Githubcontributing/fabius-design
git status
...
```bash
pnpm --filter @fabius-design/web build 2>&1 | tail -20 > /tmp/web-baseline-build.txt
```
...
```bash
pnpm --filter @fabius-design/contracts test critique.test.ts
```
...
```bash
pnpm --filter @fabius-design/contracts test sse.test.ts
```
...
```bash
pnpm --filter @fabius-design/contracts test
```
...
import type { PanelEvent } from '@fabius-design/contracts/critique';
...
```bash
pnpm --filter @fabius-design/daemon test parser.test.ts
```
...
import type { PanelEvent, PanelistRole } from '@fabius-design/contracts/critique';
...
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig } from '@fabius-design/contracts/critique';
...
```bash
pnpm --filter @fabius-design/daemon test scoreboard.test.ts
```
...
import type { PanelistRole } from '@fabius-design/contracts/critique';
...
```ts
import type { CritiqueConfig, RoundDecision } from '@fabius-design/contracts/critique';
...
```ts
import type { FallbackPolicy } from '@fabius-design/contracts/critique';
...
```bash
pnpm --filter @fabius-design/daemon test migrations.test.ts
```
...
```bash
pnpm --filter @fabius-design/daemon test orchestrator.test.ts
```
...
} from '@fabius-design/contracts/critique';
import { panelEventToSse } from '@fabius-design/contracts/sse';
import { parseCritiqueStream } from './parser';
...
```ts
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig, PROTOCOL_VERSION } from '@fabius-design/contracts/critique';
...
import { type CritiqueConfig, PROTOCOL_VERSION } from '@fabius-design/contracts/critique';
...
```bash
pnpm --filter @fabius-design/web test discovery.test.ts
```
...
import type { CritiqueSseEvent } from '@fabius-design/contracts/sse';
import type { PanelistRole } from '@fabius-design/contracts/critique';
