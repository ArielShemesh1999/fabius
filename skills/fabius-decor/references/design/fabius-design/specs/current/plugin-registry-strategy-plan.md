...
- Third parties can self-host the same `fabius-design-marketplace.json` shape as their own plugin source.
...
- `docs/plugins-spec.zh-CN.md` names CLI as the canonical API, supports headless OD, and reserves federated `fabius-design-marketplace.json`.
...
- `apps/landing-page` now has a static public `/plugins/` registry renderer and per-plugin detail routes generated from `plugins/registry/*/fabius-design-marketplace.json` plus bundled official manifests.
...
Plugin source repo
  ...
  fabius-design.json must include plugin.repo.
        |
...
Plugin artifact
  GitHub ref, GitHub Release .tgz, HTTPS archive, or local folder.
        ...
        v
Registry backend
  v1: GitHub repo + generated fabius-design-marketplace.json.
...
- **fabius-design.ai/plugins** is the public presentation of the official and community registry sources. It is equivalent to a polished static renderer over repo-owned catalog data, not a separate source of truth. `fabius-design.ai/marketplace` can remain an alias later if needed.
...
Default community registry
  ...
  -> plugin is copied to ~/.fabius-design/plugins/<plugin-id>
  -> Installed becomes part of agent context/runtime consumption
```
...
`~/.fabius-design/plugins/<plugin-id>` by default. The daemon reloads those
...
Create plugin
  ...
  -> merge regenerates fabius-design-marketplace.json
  -> users refresh Sources, see Available, install into Installed
```

The `Create plugin` product entry should therefore start an agent workflow, not a bare JSON form. The agent can gather intent, generate `SKILL.md`, `fabius-design.json`, examples, previews, and capability metadata, run local validation, install a local test copy, pack it, and drive the `gh`-backed publish flow.

High-level architecture relationship:

```text
                    fabius-design.ai/plugins
...
|                                                  |
| plugins/registry/official/fabius-design-marketplace.json |
| plugins/registry/community/fabius-design-marketplace.json |
| plugins/community/<vendor>/<plugin-name>/fabius-design.json |
| generated fabius-design-marketplace.json           |
+-----------------------------+--------------------+
                              |
...
- The generated `fabius-design-marketplace.json` is the machine-readable artifact fetched by daemon/CLI/UI.
- The per-plugin `community/**/fabius-design.json` or `entry.json` files are the human-reviewable source data used for GitHub PR governance.
- The file can initially live in the main Open Design repo if that lowers setup cost, but the product abstraction should still be `RegistryBackend`, not "read this monorepo path". Moving to `fabius-design/plugin-registry` later should be a data relocation, not a product rewrite.
...
```text
fabius-design-plugin-registry/
├── fabius-design-marketplace.json
├── community/
│   └── official/
│       ├── fabius-design-marketplace.json
│       └── plugins/
...
`fabius-design-marketplace.json` remains the portable file that third-party hosts can copy. The per-plugin `entry.json` files make PRs reviewable, reduce merge conflicts, and let the build step generate the final marketplace index deterministically.
...
- Community submissions can land as plugin source folders under `plugins/community/<vendor-or-plugin>` and be referenced by `plugins/registry/community/fabius-design-marketplace.json`; trust tier stays encoded per source/entry.
...
- The source-of-truth repo is declared in `fabius-design.json` as `plugin.repo`. Published plugins can be "anything that packs": no special repo layout is required beyond passing `od plugin validate` and `od plugin pack`, shipping a runnable skill anchor, and declaring `plugin.repo`.
- `plugin.repo` points at the canonical source repository or subdirectory, for example `https://github.com/fabius-design/plugins/tree/main/make-a-deck`. Registry entries may mirror it as `sourceRepository` for search and review without replacing the manifest field.

Minimum entry shape:

```json
{
  "name": "fabius-design/make-a-deck",
  "title": "Make a Deck",
  "version": "1.2.0",
  "distTags": {
    "latest": "1.2.0",
  ...
  "source": "github:fabius-design/plugins@3f4c2d1/make-a-deck",
  "ref": "3f4c2d1",
  "dist": {
    "type": "github",
    "archive": "https://github.com/fabius-design/plugins/archive/3f4c2d1.tar.gz",
    "integrity": "sha512-...",
    "manifestDigest": "sha256-..."
  },
  "publisher": {
    "id": "fabius-design",
    "github": "fabius-design"
  },
  "sourceRepository": "https://github.com/fabius-design/plugins/tree/main/make-a-deck",
  "homepage": "https://fabius-design.ai/plugins/fabius-design/make-a-deck/",
  "license": "MIT",
...
Required `fabius-design.json` fields for registry publish:
...
```json
{
  ...
  "version": "1.2.0",
  "plugin": {
    "repo": "https://github.com/fabius-design/plugins/tree/main/make-a-deck"
  }
}
```
...
- `LocalMarketplaceJsonBackend`: reads cached `fabius-design-marketplace.json` rows from SQLite.
...
- CLI commands must not expose GitHub-specific nouns except where the source explicitly is GitHub. `od plugin publish --to fabius-design` may use `gh` internally, but the command contract should survive a later database backend.
...
- [x] `od plugin pack`
- [ ] `od plugin publish --to fabius-design [--dry-run]`
...
- `od plugin publish --to fabius-design` validates, packs if needed, renders the registry entry, creates a branch, opens a PR, and prints the PR URL.
...
- Credentials live in `gh`, never inside `fabius-design-marketplace.json` and never inside the daemon database.
...
- [ ] Add formal `plugin.repo` schema field to `fabius-design.json` and require it for registry publish.
...
- [ ] Add `od plugin publish --to fabius-design --dry-run --json`.
...
- [x] Add public `/plugins/` route on `apps/landing-page` for fabius-design.ai: searchable official/community registry listing, static plugin detail pages, canonical/OG/Twitter metadata, JSON-LD item/detail data, and homepage/header entry points.
...
- [ ] Add migration/import from static `fabius-design-marketplace.json` and GitHub registry repo into database rows.
- [ ] Add export back to `fabius-design-marketplace.json` so enterprises can mirror or air-gap catalogs.
...
pnpm guard
pnpm typecheck
pnpm --filter @fabius-design/daemon test
pnpm --filter @fabius-design/web test
```
...
- Should the official registry live in this monorepo or a separate `fabius-design/plugin-registry` repo? Separate is cleaner for community PR review and static-site generation.
...
- Source pointer: registry-published `fabius-design.json` must include `plugin.repo`.
