...
A Home-only composer intent that lets the prompt card expose media-specific defaults before project creation. The current media surfaces are `image`, `video`, `fabius-frames`, and `audio`; they map onto the existing project kinds at submit time instead of extending the backend `ProjectKind` union.
...
**Fabius-Frames Composer Surface**:
A standalone Home composer media surface shown between Video and Audio for HTML-based motion generation. It submits as `kind: "video"` with `videoModel: "fabius-frames-html"` so persisted projects keep the existing video backend shape while the Home UI still gives Fabius-Frames its own entry point.
