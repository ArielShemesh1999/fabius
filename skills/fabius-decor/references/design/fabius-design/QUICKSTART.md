...
# Docker image tag
OPEN_DESIGN_IMAGE=docker.io/vanjayak/fabius-design:latest
...
```bash
docker volume inspect fabius-design_open_design_data
```
...
The **Design system** dropdown ships with 71 built-in systems — 2 hand-authored starters (Neutral Modern, Warm Editorial) and 69 product systems imported from [`awesome-the design corpus`](https://github.com/VoltAgent/awesome-the design corpus), grouped by category (AI & LLM, Developer Tools, Productivity, Backend, Design Tools, Fintech, E-Commerce, Media, Automotive). Pick one to skin every prototype in that brand's aesthetic, and another set of 57 design skills sourced from [`awesome-design-skills`](https://github.com/bergside/awesome-design-skills).
...
pnpm --filter @fabius-design/daemon build  # build apps/daemon/dist/cli.js for `od`
pnpm --filter @fabius-design/web build     # build the web package when needed
...
Image, video, audio, and Fabius-Frames skills call the local `od` CLI through environment variables injected by the daemon when it spawns an agent:
...
- **`better-sqlite3` fails to load / ABI mismatch after a Node.js version change** — `pnpm install` re-runs `postinstall` automatically and rebuilds the native addon for the current Node.js. To rebuild manually or verify the fix: `pnpm --filter @fabius-design/daemon rebuild better-sqlite3` then `pnpm --filter @fabius-design/daemon exec node -e "require('better-sqlite3')"`. Requires build tools: `python3`, `make`, `g++` (or `clang++`). If you have `ignore-scripts=true` in your `.npmrc`, run `node scripts/postinstall.mjs` after `pnpm install`.
