...
- Default runtime files live under `<project-root>/.tmp/<source>/<namespace>/...`; POSIX IPC sockets are fixed at `/tmp/fabius-design/ipc/<namespace>/<app>.sock`.
...
maintenance surface. Do not recreate `tools/pr`, `@fabius-design/tools-pr`, or a
...
- CSS refactors must preserve cascade semantics. For mechanical splits, verify expanded import content/order matches the previous stylesheet; for CSS Module migrations, validate the affected UI path with `pnpm --filter @fabius-design/web typecheck` and a focused build/test or visual check when practical.
...
```bash
pnpm --filter @fabius-design/web typecheck
pnpm --filter @fabius-design/web test
pnpm --filter @fabius-design/web build
pnpm --filter @fabius-design/daemon test
pnpm --filter @fabius-design/daemon build
pnpm --filter @fabius-design/desktop build
pnpm --filter @fabius-design/tools-dev build
pnpm --filter @fabius-design/tools-pack build
pnpm --filter @fabius-design/tools-serve build
```
...
`@fabius-design/sidecar-proto` owns Open Design app/mode/source constants, namespace validation, stamp fields/flags, IPC message schema, status shapes, and error semantics. `@fabius-design/sidecar` provides only generic bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime files. `@fabius-design/platform` provides only generic OS process stamp serialization, command parsing, and process matching/search primitives, consuming the proto descriptor.
