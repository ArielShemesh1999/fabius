---
name: fabius-decor-gsap-performance
description: GSAP animation performance — prefer transforms, avoid layout thrashing, will-change, quickTo for frequent updates. Use when optimizing GSAP animations, reducing jank, or targeting smooth 60fps.
license: MIT
---
...
Use **will-change** in CSS on elements that will animate to hint the browser to promote the layer:

```css
will-change: transform;
```

Only apply to elements that are actually animating; setting it everywhere wastes GPU memory.

## Batch Reads and Writes

GSAP batches updates internally. When mixing GSAP with direct DOM reads/writes, avoid interleaving reads and writes in a way that causes repeated layout thrashing. Do all reads first, then all writes (or let GSAP handle writes in one pass).
...
- Avoid animating many properties on many elements at once; simplify or sequence if needed.
...
- ❌ Set **will-change** on every element "just in case"; use only for elements that are actually animating.
- ❌ Create hundreds of overlapping tweens or ScrollTriggers without testing on low-end devices.
- ❌ Ignore cleanup; stray tweens and ScrollTriggers keep running and hurt performance and correctness.
