---
name: fabius-decor-gsap-timeline
description: GSAP timelines — gsap.timeline(), position parameter, labels, nesting, playback control. Use when sequencing multiple animations, choreographing keyframes, or when the user asks about animation sequencing or timeline control.
license: MIT
---
...
- ✅ Prefer timelines for sequencing multi-step animations.
...
- ❌ Forget to pass **defaults** when many child tweens share the same duration or ease.
- ❌ Confuse `duration` on the timeline constructor with tween duration; timeline duration is determined by its children.
