---
name: fabius-decor-hyperframes-media
description: Asset preprocessing for HyperFrames — text-to-speech narration (Kokoro), audio/video transcription (Whisper), and background removal for transparent overlays (u2net). Use when generating voiceover, transcribing speech for captions, removing video backgrounds, choosing a TTS voice or Whisper model, or chaining TTS → transcribe → captions. Each command downloads its model on first run.
---

# HyperFrames Media Preprocessing

Three CLI commands that produce composition assets: `tts` (speech), `transcribe` (timestamps), and `remove-background` (transparent video). Each downloads a model on first run and caches it under `~/.cache/hyperframes/`. Drop the output into the project, then reference it from the composition HTML — see the `hyperframes` skill for audio/video element conventions.
