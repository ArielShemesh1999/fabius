...
description: Author and ship a new HyperFrames registry block or component as a PR — from idea to merged catalog entry. Use ONLY when contributing to the public catalog. For in-project authoring use the `hyperframes` skill; for installing existing items use the `hyperframes-registry` skill.
---
...
- **Block** (`registry/blocks/`, type `hyperframes:block`) — full standalone composition with fixed dimensions and duration. Caption styles, VFX effects, title cards, lower thirds.
- **Component** (`registry/components/`, type `hyperframes:component`) — reusable snippet with no fixed dimensions or duration. CSS effects, text treatments, overlays that adapt to any composition size.
