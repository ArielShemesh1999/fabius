---
name: fabius-decor-gsap-frameworks
description: GSAP with Vue, Svelte, and other non-React frameworks — lifecycle, scoping selectors, cleanup on unmount. Use when animating in Vue, Nuxt, Svelte, or SvelteKit with GSAP. For React use gsap-react.
license: MIT
---
...
- ✅ **gsap.context(scope)** — pass the container ref (`container.value`) as the second argument so selectors like `.item` are scoped to that root. All animations and ScrollTriggers created inside the callback are tracked and reverted when **ctx.revert()** is called.
...
Use **onMount** to run GSAP after the DOM is ready. Return the cleanup function from onMount (or track the context and clean up on destroy). Svelte 5 uses a different lifecycle; the same principle applies: create in "mounted" and revert in "destroyed."
...
- ✅ **return () => ctx.revert()** — Svelte's onMount accepts a cleanup return; call **ctx.revert()** there so cleanup runs when the component is destroyed.

## Scoping Selectors

Always pass the **scope** (container element or ref) as the second argument to **gsap.context(callback, scope)** so selectors inside the callback are limited to that subtree.
...
ScrollTrigger instances are included in **gsap.context()** and reverted when **ctx.revert()** is called. Create ScrollTriggers inside the same **gsap.context()** callback used for tweens. Call **ScrollTrigger.refresh()** after layout changes that affect trigger positions (e.g. after data loads); in Vue/Svelte that often means after DOM updates (nextTick in Vue, tick in Svelte, or after async content load).
...
Do not create GSAP animations before the root element exists. Wait for **onMounted** / **onMount** (or equivalent) so the container ref is in the DOM.
