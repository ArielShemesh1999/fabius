---
name: fabius-decor-gsap-plugins
description: GSAP plugins — registration, ScrollToPlugin, ScrollSmoother, Flip, Draggable, Inertia, Observer, SplitText, ScrambleText, SVG plugins, physics plugins, CustomEase, EasePack, GSDevTools. Use when the user asks about a GSAP plugin, scroll-to, flip animations, draggable, SVG drawing, or plugin registration.
license: MIT
---
...
- ✅ In React, register at top level or once in the app (before first useGSAP); do not register inside a component that re-renders. useGSAP is a plugin that needs to be registered before use.
...
Smooth scroll wrapper (smooths native scroll). Requires ScrollTrigger and a specific DOM structure (content wrapper + smooth wrapper). Use when smooth, momentum-style scroll is needed. Register after ScrollTrigger. DOM structure:
...
Capture state with `Flip.getState()`, apply changes (e.g. layout or class changes), then use `Flip.from()` to animate from the previous state to the new state (FLIP: First, Last, Invert, Play). Use when animating between two layout states (lists, grids, expanded/collapsed).
...
Works with Draggable for momentum after release, or tracks velocity of any property so it can glide to a stop. Register with Draggable when using `inertia: true`:
...
Splits an element's text into characters, words, and/or lines (each in its own element) for staggered or per-unit animation. Returns an instance with **chars**, **words**, **lines** (and **masks** when `mask` is set). Restore original markup with **revert()** or let **gsap.context()** revert. Integrates with **gsap.context()**, **matchMedia()**, and **useGSAP()**. API: **SplitText.create(target, vars)** (target = selector, element, or array).
...
| **aria** | `"auto"` (default), `"hidden"`, or `"none"`. `"auto"` adds `aria-label` on the split element and `aria-hidden` on line/word/char elements so screen readers read the label; `"hidden"` hides all from readers; `"none"` leaves aria unchanged. |
| **autoSplit** | When `true`, reverts and re-splits when fonts finish loading or when the element width changes. Animations must be created inside **onSplit()** so they target newly split elements; return the animation from **onSplit()** for automatic cleanup and time-sync on re-split. |
| **onSplit(self)** | Callback when split completes (and on each re-split if **autoSplit** is `true`). Receives the SplitText instance. Returning a GSAP tween or timeline enables automatic revert/sync on re-splitting. |
...
| **tag** | Wrapper element tag; default `"div"`. Use `"span"` for inline (note: transforms may not render on inline elements in some browsers). |
...
| **wordDelimiter** | Word boundary: string (default `" "`), RegExp, or `{ delimiter: RegExp, replaceWith: string }` for custom splitting. |
| **prepareText(text, parent)** | Function that receives raw text and parent element; return modified text before splitting. |
| **propIndex** | When `true`, adds a CSS variable with index on each split element (e.g. `--word: 1`, `--char: 2`). |
| **reduceWhiteSpace** | Collapse consecutive spaces; default `true`. From v3.13.0 also honors line breaks. |
| **onRevert** | Callback when the instance is reverted. |

**Tips:** Split only what is animated (skip chars if only animating words). For custom fonts, split after they load (`document.fonts.ready.then(...)`) or use **autoSplit: true** with **onSplit()**. To avoid kerning shift when splitting chars, use CSS `font-kerning: none; text-rendering: optimizeSpeed;`. Avoid `text-wrap: balance`; it can interfere with splitting. SplitText does not support SVG `<text>`.
...
**drawSVG value:** Describes the **visible segment** of the stroke along the path. Format: `"start end"` in percent or length. Examples: `"0% 100%"` = full stroke; `"20% 80%"` = stroke only between 20% and 80%. Single value (e.g. `"100%"`) means start is 0. The tween animates from the element's **current** segment to the **target** segment.
...
**Caveats:** Only affects stroke (not fill). Prefer single-segment `<path>` elements; multi-segment paths can render oddly in some browsers. **DrawSVGPlugin.getLength(element)** and **DrawSVGPlugin.getPosition(element)** return stroke length and current position.

**Learn more:** [DrawSVG](https://gsap.com/docs/v3/Plugins/DrawSVGPlugin)

### MorphSVG (MorphSVGPlugin)

Morphs one SVG shape into another by animating the `d` attribute (path data). Start and end shapes do not need the same number of points — MorphSVG converts to cubic beziers and adds points as needed. Works on `<path>`, `<polyline>`, and `<polygon>`; `<circle>`, `<rect>`, `<ellipse>`, and `<line>` are converted via **MorphSVGPlugin.convertToPath(selector | element)**.

**morphSVG value:** A **selector**, **element**, **raw path data** string, or for polygon/polyline a **points string**. For full config use the **object form** with **shape** as the only required property.
...
| **type** | `"linear"` (default) or `"rotational"`. Rotational uses angle/length interpolation; try it when linear looks wrong. |
| **map** | How segments are matched: `"size"` (default), `"position"`, or `"complexity"`. |
| **shapeIndex** | Offsets which point in the start path maps to the first point in the end path (avoids shape "crossing over"). Number for single-segment paths; **array** for multi-segment. Use **shapeIndex: "log"** once to log the auto-calculated value. Only applies to closed paths. |
| **smooth** | (v3.14+). Adds smoothing points. Number, `"auto"`, or object: `{ points, redraw, persist }`. Use when the default morph looks jagged. |
| **curveMode** | Boolean (v3.14+). Interpolates control-handle angle/length to avoid kinks on curves. |
| **origin** | Rotation origin for **type: "rotational"**. String: `"50% 50%"` (default). |
| **precision** | Decimal places for output path data; default `2`. |
| **precompile** | Array of precomputed path strings (or `"log"` once, copy from console). Skips expensive startup calculations for very complex morphs. |
| **render** | Function(rawPath, target) called each update — e.g. draw to canvas. |
| **updateTarget** | When using **render** (e.g. canvas-only), set `false` so the original `<path>` is not updated. |

**Utilities:** **MorphSVGPlugin.convertToPath(selector | element)** converts circle/rect/ellipse/line/polygon/polyline to `<path>` in the DOM. **MorphSVGPlugin.rawPathToString(rawPath)** and **stringToRawPath(d)** convert between path strings and raw arrays.

**Tips:** For twisted or inverted morphs, set **shapeIndex** (use `"log"` or findShapeIndex()). For multi-segment paths, **shapeIndex** is an array (one value per segment). Precompile only when the first frame is slow.
