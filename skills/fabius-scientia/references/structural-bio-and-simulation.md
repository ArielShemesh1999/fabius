...
- **Confidence is calibration, not correctness — know the failure envelope before you trust a complex.** A high score is not a pass mark, and on the hard cases the score stops discriminating: low-homology benchmarking of the all-atom co-folding models (AlphaFold 3, Boltz, Chai, HelixFold3, Protenix over 1,522 assemblies) finds antibody-antigen complexes failing more than half the time with per-sample confidence *unable* to identify the near-native pose, ligand pose accuracy decaying as the ligand's similarity to the training set falls, allosteric pockets routinely mis-sited because training data over-represents orthosteric binding, and nucleic-acid structures trailing everything else. So before you report a predicted complex, state three things alongside the score: (a) whether the target postdates the model's training cutoff, (b) how novel the ligand or pocket is relative to the PDB, and (c) the **interface** metric (DockQ / ipTM), not a mean pLDDT that averages a rigid core over the interface you actually care about.
- **Benchmark on a low-homology set, not a recent one.** Filtering a test set by deposition date only proves the target is new; filtering it by sequence identity, ligand similarity and interface TM-score proves the *model* is generalizing. A date-filtered-but-homologous set measures memorization and reports it as accuracy.
