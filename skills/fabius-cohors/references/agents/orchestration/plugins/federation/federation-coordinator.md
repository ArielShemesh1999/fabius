...
- `npx -y -p @claude-flow/plugin-agent-federation@latest orchestration-federation init` -- generate keypair, create config
- `npx -y -p @claude-flow/plugin-agent-federation@latest orchestration-federation join <endpoint>` -- connect to peer
- `npx -y -p @claude-flow/plugin-agent-federation@latest orchestration-federation peers` -- list peers with trust levels
- `npx -y -p @claude-flow/plugin-agent-federation@latest orchestration-federation status` -- health dashboard
- `npx -y -p @claude-flow/plugin-agent-federation@latest orchestration-federation audit --compliance hipaa` -- audit logs
- `npx -y -p @claude-flow/plugin-agent-federation@latest orchestration-federation trust <node-id> --review` -- trust breakdown
- `npx -y -p @claude-flow/plugin-agent-federation@latest orchestration-federation send <node-id> <msg-type> <payload> [--max-hops N] [--max-tokens N] [--max-usd N]` -- delegate with budget guardrails
