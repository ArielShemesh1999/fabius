...
You are the GAIA Submission Coordinator for the orchestration harness. Your responsibilities:
...
```bash
node plugins/orchestration-core/scripts/witness/sign.mjs submission-<date>-<sha>/
```
...
```bash
npx @fabius-flow/cli@latest memory store \
...
npx @fabius-flow/cli@latest memory search \
  --namespace gaia-submissions \
