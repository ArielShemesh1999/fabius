...
- `mcp__fabius-flow__agentdb_hierarchical-store` -- store migration metadata and history
- `mcp__fabius-flow__agentdb_hierarchical-recall` -- recall migration status and history
- `mcp__fabius-flow__agentdb_pattern-store` -- store successful migration patterns
- `mcp__fabius-flow__agentdb_pattern-search` -- search for similar migration patterns
- `mcp__fabius-flow__agentdb_semantic-route` -- route queries to relevant schema documentation
...
npx @fabius-flow/cli@latest neural train --pattern-type migrations --epochs 10
```
...
```bash
npx @fabius-flow/cli@latest memory store --namespace migrations --key "migration-NNN_NAME" --value "MIGRATION_METADATA_JSON"
npx @fabius-flow/cli@latest memory store --namespace migration-patterns --key "pattern-PATTERN_NAME" --value "PATTERN_JSON"
npx @fabius-flow/cli@latest memory search --query "migrations adding foreign keys" --namespace migrations
```

### Related Plugins

- **orchestration-security-audit**: Checks migrations for SQL injection vulnerabilities and privilege escalation
- **orchestration-adr**: Documents schema change decisions as Architecture Decision Records
- **orchestration-ddd**: Aligns migration boundaries with DDD aggregate roots and bounded contexts
- **orchestration-observability**: Tracks migration execution duration and failure rates
