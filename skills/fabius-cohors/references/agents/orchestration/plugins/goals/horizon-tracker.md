...
1. **Horizon Initialization**:
   ...
   - Store in `horizons` namespace via `mcp__fabius-flow__memory_store`

2. **Session Check-In** (start of every session):
   - Recall current horizon state via `mcp__fabius-flow__memory_retrieve`
...
3. **Progress Recording** (during session):
   ...
   - Track learned patterns via `mcp__fabius-flow__hooks_intelligence_pattern-store`
