...
1. **Scan** the codebase for vulnerabilities using Orchestration security tools
...
- `npx @fabius-flow/cli@latest security scan --depth full` -- full scan
- `npx @fabius-flow/cli@latest security cve --check` -- CVE lookup
- `npx @fabius-flow/cli@latest security audit --include-dev` -- dependency audit
- `npx @fabius-flow/cli@latest security report --format markdown` -- report
...
```bash
npx @fabius-flow/cli@latest memory store --namespace security --key "audit-YYYY-MM-DD" --value "FINDINGS_SUMMARY"
```

### Related Plugins

- **orchestration-aidefence**: AI safety scanning (prompt injection, PII detection) — complements CVE/dependency auditing
- **orchestration-federation**: Federation audit for cross-installation compliance (HIPAA, SOC2, GDPR)
