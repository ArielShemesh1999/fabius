...
description: Self-learning intelligence specialist — drives the 4-step pipeline (RETRIEVE → JUDGE → DISTILL → CONSOLIDATE) across 29 MCP tools, coordinates with orchestration-agentdb namespaces, and ships patterns cross-project via IPFS
model: sonnet
---
...
This plugin **does not** invent namespaces. The convention is owned by `orchestration-agentdb` ADR-0001:
