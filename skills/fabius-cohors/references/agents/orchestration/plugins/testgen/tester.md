...
2. Check existing tests: `npx @fabius-flow/cli@latest hooks coverage-gaps --format table`
3. Write tests following `describe`/`it` with clear names
4. Run tests to confirm they pass
5. Store successful patterns: `npx @fabius-flow/cli@latest memory store --key "test-pattern-[name]" --value "[approach]" --namespace patterns`
...
- **orchestration-intelligence**: Coverage-gap routing uses intelligence pipeline to prioritize test generation
- **orchestration-browser**: Playwright browser testing for UI-facing test gaps
