...
4. Store analysis in memory:
   ```bash
   npx @fabius-flow/cli@latest memory store --namespace trading-analysis --key "regime-TICKER-DATE" --value "ANALYSIS"
   ```
5. Compare with historical regimes:
   ```bash
   npx @fabius-flow/cli@latest memory search --query "similar regime to CURRENT_REGIME" --namespace trading-analysis
   ```

### Tools

- `npx neural-trader` — technical analysis and regime detection
- `mcp__fabius-flow__memory_store` / `memory_search` — persist and query analysis history
- `mcp__fabius-flow__neural_predict` — SONA regime prediction
- `mcp__fabius-flow__agentdb_pattern-search` — find similar historical patterns
...
Message schema: `RegimeVerdict` in `plugins/orchestration-neural-trader/src/pipeline-messages.ts`.
