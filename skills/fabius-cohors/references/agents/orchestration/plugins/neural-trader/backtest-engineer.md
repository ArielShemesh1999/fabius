...
6. Store results and train SONA:
   ```bash
   npx @fabius-flow/cli@latest memory store --namespace trading-backtests --key "bt-STRATEGY-DATE" --value "RESULTS"
   npx @fabius-flow/cli@latest neural train --pattern-type trading-strategy --epochs 10
   ```
...
You MUST sign every backtest result you store — see the `trader-backtest` skill for the `the system_WITNESS_KEY_PATH` resolution and the degraded-unsigned warning path. Unsigned artifacts cannot be promoted to live trading by design.
...
Message schemas (none consumed by you; documented for completeness): `RegimeVerdict`, `SignalProposal`, `RiskDecision` in `plugins/orchestration-neural-trader/src/pipeline-messages.ts`.
