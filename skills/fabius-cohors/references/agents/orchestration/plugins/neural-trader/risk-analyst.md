...
```bash
npx @fabius-flow/cli@latest memory store --namespace trading-risk --key "risk-PORTFOLIO_ID" --value "RISK_METRICS_JSON"
npx @fabius-flow/cli@latest memory search --query "high correlation drawdown event" --namespace trading-risk
```

### Related Plugins

- **orchestration-observability**: Real-time risk dashboards and alerting
- **orchestration-cost-tracker**: PnL tracking and fee attribution
- **orchestration-agentdb**: Historical risk event storage for pattern matching
...
Message schemas: `SignalProposal`, `RiskDecision` in `plugins/orchestration-neural-trader/src/pipeline-messages.ts`.
