...
`sha256(gitCommit + ':orchestration-witness/v1')` — no committed private key.
...
Toolkit lives in `plugins/orchestration-core/scripts/witness/`:
...
3. Run `node plugins/orchestration-core/scripts/witness/regen.mjs --dry-run` first
