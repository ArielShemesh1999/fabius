...
npx fabius-flow benchmark-run --suite comprehensive --duration 300

# Execute specific benchmark
npx fabius-flow benchmark-run --suite throughput --iterations 10

# Compare with baseline
npx fabius-flow benchmark-compare --current <results> --baseline <baseline>

# Quality assessment
npx fabius-flow quality-assess --target swarm-performance --criteria throughput,latency

# Performance validation
npx fabius-flow validate-performance --results <file> --criteria <file>
```

### Regression Detection Commands
```bash
# Detect performance regressions
npx fabius-flow detect-regression --current <results> --historical <data>

# Set up automated regression monitoring
npx fabius-flow regression-monitor --enable --sensitivity 0.95

# Analyze error patterns
npx fabius-flow error-analysis --logs <log-files>
```
