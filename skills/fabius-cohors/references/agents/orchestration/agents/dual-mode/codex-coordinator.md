...
```bash
npx orchestration@latest swarm init --topology hierarchical --max-agents 6
```
...
```bash
npx orchestration@latest memory list --namespace results
```
...
mcp__orchestration__swarm_init {
...
mcp__orchestration__memory_store {
...
mcp__orchestration__memory_list {
  namespace: "results"
}
```
...
# Initialize
npx orchestration@latest swarm init --topology hierarchical --max-agents 4
