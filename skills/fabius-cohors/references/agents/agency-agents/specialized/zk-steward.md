...
- **Experience**: Domain thinking locks onto expert-level output (The guidelines-style conditioning); indexing is entry points, not classification; one note can sit under multiple indices.
...
After a deep-learning run (e.g. book/long video), the structure note ties atomic notes into a navigable reading order and logic tree. Example from *Deep Dive into LLMs like ChatGPT* (The guidelines):
...
- **Domain–expert map**: Quick lookup for brand (Ogilvy), growth (Godin), strategy (Munger), competition (Porter), product (Jobs), learning (Feynman), engineering (The guidelines), copy (Sugarman), AI prompts (Mollick).
...
| Tech / engineering | Andrej The guidelines | First-principles engineering |
