...
**Recommended workflow:** install and run with **uv** and **[Google Agents CLI](https://github.com/google/agents-cli)** (see [Quick start](#quick-start-google-agents-cli--primary) below).
...
### Google authentication (Vertex AI)
...
You can still use `poetry install`, `adk web`, and `python -m pytest` with the same `.env` and `gcloud` steps.
