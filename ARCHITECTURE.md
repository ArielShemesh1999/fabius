...
- **`references/`** — on-demand depth bundled per layer, loaded only when a layer needs it so the skills stay lean. Each specialist ships a lean entry doc **plus a full library**: `fabius-decor` → a 69-brand design teardown library (`references/design/`) + the figura visualization entry (`references/visualization.md`); `fabius-cohors` → a 200+-agent production catalog with a `fabius-vec.db` memory index (`references/agents/`); `fabius-archivum` → a knowledge engine, wiki pattern, and RAG pipeline, carried as design reference rather than a runnable dependency (`references/knowledge/`); `fabius-disciplina` → the craft + discipline process library (`references/process/`); `fabius-mercatus` → a channel + swipe library (`references/channel-swipe-library.md`); `fabius-praesidium` → a hardening + audit library (`references/hardening-guides.md`); `fabius-ludus` → an engine + feel library (`references/engine-recipes.md`); `fabius-catena` → the on-chain dev + sealing playbooks (`references/onchain-playbook.md` + `references/sealing.md`); `fabius-machina` → the automation build-and-verify playbook (`references/automation-playbook.md`); `fabius-scientia` → the scientific-method + database-lookup playbook (`references/science-playbook.md`); `fabius-doctrina` → the ML-engineering playbook — serving, MLOps, evaluation (`references/ml-engineering-playbook.md`); `fabius-fortuna` → the markets & quant playbook — analysis, valuation, honest backtesting, risk (`references/markets-and-quant-playbook.md`); `fabius-concilium` → the council protocol + a zero-dependency runnable reference (`references/council-protocol.md` + `references/council.mjs`); `fabius-parcus` → the lean guidelines (`references/lean/`).
- **`runtime/`** — the local body. A zero-dependency Node CLI that runs the same router, the same rules and the same contracts on the user's own machine: it reads the routed `SKILL.md` files off disk (and reports whether they still match the seal), gates every tool behind a capability model over a symlink-resolved working-directory jail, runs the delivered artifact against the real toolchain as a hard verification oracle, keeps memory as plain markdown, audits an external surface with no API key (`fabius recon`), and can be reached over an encrypted keypair channel with no server in between (`fabius listen`). The model call is injectable, so the whole loop is covered by 75 checks that spend nothing — 68 fully offline, and 7 specification-vector tests that skip loudly until `npm run vectors` fetches the BIP-340 / NIP-44 vectors once. Design contract → `skills/fabius-cohors/references/local-agent-runtime.md`.
...
- **`evals/`** — the benchmark *and* the structural gate; [`evals/README.md`](evals/README.md) is the index. `structural.mjs` — 23 deterministic invariants over the repo itself (counts, frontmatter budgets, reference integrity, sealed-set drift, the Merkle seal), no model, no key, no network (**23/23**). `suite/` — the Fabius Benchmark Suite v1.0: 100 neutral tasks in three tiers (20 smoke / 50 core / 30 stress) across ten categories A–J, run in three modes (BASE / FAB / FAB_MEMORY), with its own deterministic validator (`node evals/suite/validate.mjs`, **9/9**). The panel harnesses `harness.v5.workflow.js` (quality), `harness.v6.workflow.js` (objective), `harness.v7.workflow.js` (the FBS run), plus `eval.mjs` (the no-system-prompt Anthropic/OpenAI baseline), `portable_eval.py` (cross-vendor: OpenAI · xAI · Mistral · Anthropic · Gemini, stdlib only) and the base `harness.workflow.js`. The canonical receipt `results.benchmark.json` and the panel receipts `results.v3/v5/v6/v7.json` are committed; only the per-run `results.json` is gitignored. Method and measured numbers → [BENCHMARKS.md](BENCHMARKS.md).
...
The other six skills — the router `fabius` itself, plus `parcus`, `decor`, `mercatus`, `praesidium`, `ludus` — need no external connection at all. Nothing here is bundled; each live tier is the user's to wire — the same lean rule everywhere: **hold the pattern, add the runtime only when the task demands it.**
...
- **Add a benchmark task or model** — the suite is versioned and count-locked (`evals/suite/validate.mjs` asserts exactly 20/50/30 and the per-category balance), so a task is *swapped* inside its tier in `evals/suite/tier1.smoke.jsonl` / `tier2.core.jsonl` / `tier3.stress.jsonl`; growing the suite means bumping those counts and cutting FBS v1.1. Re-run `node evals/suite/validate.mjs` (9/9) either way. A model goes in the relevant panel harness (`evals/harness.v5/v6/v7.workflow.js`), or in `evals/eval.mjs` for the no-system-prompt baseline. A new structural invariant goes in `evals/structural.mjs` — bump the 23/23 count everywhere it is quoted.
