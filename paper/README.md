...
> **Scope note (v1.1).** The paper proves the **routing core** — the router, the lean core, and the original specialists. fabius v1.1 adds three more domain verticals (`fabius-catena`, `fabius-machina`, `fabius-scientia`) that plug into the *same* single-owner contract and routing policy (`R1` Domain axis, `R13` verticals) **without changing the proven core** — so every theorem in the PDF still holds as written. The architecture-of-record for the current twelve-skill system is [`../ARCHITECTURE.md`](../ARCHITECTURE.md); a future paper revision will fold the verticals into the formal treatment.
